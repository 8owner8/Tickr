...
using Tickr.OfficialPlugins.ItemsMatcher.Data;
using Tickr.Steam;
...
namespace Tickr.OfficialPlugins.ItemsMatcher;
...
internal sealed class RemoteCommunication : IAsyncDisposable, IDisposable {
	...
	internal RemoteCommunication(Bot bot) {
		...
		Bot = bot;
		WebBrowser = new WebBrowser(bot.TickrLogger, TickrApp.GlobalConfig?.WebProxy, true);
		...
		if (Bot.BotConfig.TradingPreferences.HasFlag(BotConfig.ETradingPreferences.SteamTradeMatcher) && Bot.BotConfig.RemoteCommunication.HasFlag(BotConfig.ERemoteCommunication.PublicListing)) {
			HeartBeatTimer = new Timer(
				OnHeartBeatTimer,
				null,
				TimeSpan.FromMinutes(1) + TimeSpan.FromSeconds(TickrApp.LoadBalancingDelay * Bot.Bots?.Count ?? 0),
				TimeSpan.FromMinutes(1)
			);
		}

		if (Bot.BotConfig.TradingPreferences.HasFlag(BotConfig.ETradingPreferences.MatchActively)) {
			if ((TickrApp.GlobalConfig?.LicenseID != null) && (TickrApp.GlobalConfig.LicenseID != Guid.Empty)) {
				MatchActivelyTimer = new Timer(
					MatchActively,
					null,
					TimeSpan.FromHours(1) + TimeSpan.FromSeconds(TickrApp.LoadBalancingDelay * Bot.Bots?.Count ?? 0),
					TimeSpan.FromHours(6)
				);
			} else {
				bot.TickrLogger.LogGenericError(Strings.FormatWarningNoLicense(nameof(BotConfig.ETradingPreferences.MatchActively)));
			}
		}
	}
	...
	internal async Task OnPersonaState(string? nickname = null, string? avatarHash = null) {
		...
		try {
			...
			if (!eligible.HasValue) {
				...
				Bot.TickrLogger.LogGenericWarning(Strings.FormatWarningFailedWithError($"{nameof(IsEligibleForListing)}: {eligible?.ToString() ?? "null"}"));
			...
			if (!eligible.Value) {
				...
				Bot.TickrLogger.LogGenericWarning(Strings.FormatWarningFailedWithError($"{nameof(IsEligibleForListing)}: {eligible.Value}"));
			...
			string? tradeToken = await Bot.TickrHandler.GetTradeToken().ConfigureAwait(false);

			if (string.IsNullOrEmpty(tradeToken)) {
				// This is actually a network failure, so we'll stop sending heartbeats but not record it as valid check
				ShouldSendHeartBeats = false;

				Bot.TickrLogger.LogGenericWarning(Strings.FormatWarningFailedWithError(nameof(tradeToken)));
			...
			try {
				inventory = await Bot.TickrHandler.GetMyInventoryAsync().ToListAsync().ConfigureAwait(false);
				...
				Bot.TickrLogger.LogGenericException(e);
			...
			if (!SignedInWithSteam) {
				HttpStatusCode? signInWithSteam = await TickrNet.SignInWithSteam(Bot, WebBrowser).ConfigureAwait(false);
			...
			if (!matchEverything) {
				// We should deduplicate our sets before sending them to the server, for doing that we'll use backend set parts data
				...
				if (setPartsResponse == null) {
					// This is actually a network failure, so we'll stop sending heartbeats but not record it as valid check
					ShouldSendHeartBeats = false;
					Bot.TickrLogger.LogGenericWarning(Strings.FormatErrorObjectIsNull(nameof(setPartsResponse)));
				...
				if (setPartsResponse.StatusCode.IsRedirectionCode()) {
					ShouldSendHeartBeats = false;
					Bot.TickrLogger.LogGenericWarning(Strings.FormatWarningFailedWithError(setPartsResponse.StatusCode));

					if (setPartsResponse.FinalUri.Host != TickrWebHandler.SteamCommunityURL.Host) {
						TickrApp.TickrLogger.LogGenericError(Strings.FormatWarningUnknownValuePleaseReport(nameof(setPartsResponse.FinalUri), setPartsResponse.FinalUri));
				...
				if (!setPartsResponse.StatusCode.IsSuccessCode()) {
					// TickrNet told us that we've sent a bad request, so the process should restart from the beginning at later time
					ShouldSendHeartBeats = false;
					Bot.TickrLogger.LogGenericWarning(Strings.FormatWarningFailedWithError(setPartsResponse.StatusCode));

					switch (setPartsResponse.StatusCode) {
						case HttpStatusCode.Forbidden:
							// TickrNet told us to stop submitting data for now
							LastAnnouncement = DateTime.UtcNow.AddYears(1);

							return;
						case HttpStatusCode.TooManyRequests:
							// TickrNet told us to try again later
							LastAnnouncement = DateTime.UtcNow.AddDays(1);
				...
				if (setPartsResponse.Content?.Result == null) {
					...
					Bot.TickrLogger.LogGenericError(Strings.FormatWarningUnknownValuePleaseReport(nameof(setPartsResponse), setPartsResponse.Content?.Result));
			...
			if (assetsForListing.Count > MaxItemsCount) {
				...
				Bot.TickrLogger.LogGenericWarning(Strings.FormatWarningFailedWithError($"{nameof(assetsForListing)} > {MaxItemsCount}"));
			...
			if ((BotCache.LastAnnouncedAssetsForListing.Count > 0) && !string.IsNullOrEmpty(previousChecksum)) {
				...
				if (diffCheckResponse == null) {
					// This is actually a network failure, so we'll stop sending heartbeats but not record it as valid check
					ShouldSendHeartBeats = false;
					Bot.TickrLogger.LogGenericWarning(Strings.FormatErrorObjectIsNull(nameof(diffCheckResponse)));
				...
				if (diffCheckResponse.StatusCode.IsRedirectionCode()) {
					ShouldSendHeartBeats = false;
					Bot.TickrLogger.LogGenericWarning(Strings.FormatWarningFailedWithError(diffCheckResponse.StatusCode));

					if (diffCheckResponse.FinalUri.Host != TickrWebHandler.SteamCommunityURL.Host) {
						TickrApp.TickrLogger.LogGenericError(Strings.FormatWarningUnknownValuePleaseReport(nameof(diffCheckResponse.FinalUri), diffCheckResponse.FinalUri));
				...
				switch (diffCheckResponse.Content?.Result) {
					case null:
						// This should never happen if we got the correct response
						Bot.TickrLogger.LogGenericError(Strings.FormatWarningUnknownValuePleaseReport(nameof(diffCheckResponse), diffCheckResponse.Content?.Result));
					...
					case true: {
						...
						Bot.TickrLogger.LogGenericInfo(Localization.Strings.FormatListingAnnouncing(Bot.SteamID, nickname ?? Bot.SteamID.ToString(CultureInfo.InvariantCulture), assetsForListing.Count));
						...
						for (byte i = 0; i < WebBrowser.MaxTries; i++) {
							...
							if (diffResponse == null) {
								// This is actually a network failure, so we'll stop sending heartbeats but not record it as valid check
								ShouldSendHeartBeats = false;
								Bot.TickrLogger.LogGenericWarning(Strings.FormatErrorObjectIsNull(nameof(diffResponse)));
							...
							if (diffResponse.StatusCode.IsRedirectionCode()) {
								ShouldSendHeartBeats = false;
								Bot.TickrLogger.LogGenericWarning(Strings.FormatWarningFailedWithError(diffResponse.StatusCode));

								if (diffResponse.FinalUri.Host != TickrWebHandler.SteamCommunityURL.Host) {
									TickrApp.TickrLogger.LogGenericError(Strings.FormatWarningUnknownValuePleaseReport(nameof(diffResponse.FinalUri), diffResponse.FinalUri));
							...
							if (!diffResponse.StatusCode.IsSuccessCode()) {
								...
								switch (diffResponse.StatusCode) {
									case HttpStatusCode.Conflict:
										// TickrNet told us to do full announcement instead, the only non-OK response we accept
										break;
									case HttpStatusCode.Forbidden:
										// TickrNet told us to stop submitting data for now
										LastAnnouncement = DateTime.UtcNow.AddYears(1);
							...
							if (diffResponse.Content?.Result == null) {
								// This should never happen if we got the correct response
								Bot.TickrLogger.LogGenericError(Strings.FormatWarningUnknownValuePleaseReport(nameof(diffResponse), diffResponse.Content?.Result));
						...
						if (diffResponse == null) {
							...
							Bot.TickrLogger.LogGenericWarning(Strings.FormatWarningFailedWithError(nameof(diffResponse)));
						...
						if (diffResponse.StatusCode.IsSuccessCode() && diffResponse.Content is { Success: true, Result.Finished: true }) {
							...
							Bot.TickrLogger.LogGenericInfo(Strings.Success);
			...
			for (byte i = 0; i < WebBrowser.MaxTries; i++) {
				...
				if (announceResponse == null) {
					// This is actually a network failure, so we'll stop sending heartbeats but not record it as valid check
					ShouldSendHeartBeats = false;
					Bot.TickrLogger.LogGenericWarning(Strings.FormatErrorObjectIsNull(nameof(announceResponse)));
				...
				if (announceResponse.StatusCode.IsRedirectionCode()) {
					ShouldSendHeartBeats = false;
					Bot.TickrLogger.LogGenericWarning(Strings.FormatWarningFailedWithError(announceResponse.StatusCode));

					if (announceResponse.FinalUri.Host != TickrWebHandler.SteamCommunityURL.Host) {
						TickrApp.TickrLogger.LogGenericError(Strings.FormatWarningUnknownValuePleaseReport(nameof(announceResponse.FinalUri), announceResponse.FinalUri));
				...
				if (!announceResponse.StatusCode.IsSuccessCode()) {
					...
					switch (announceResponse.StatusCode) {
						case HttpStatusCode.Conflict:
							// TickrNet told us to that we've applied wrong deduplication logic, we can try again in a second
				...
				if (announceResponse.Content?.Result == null) {
					// This should never happen if we got the correct response
					Bot.TickrLogger.LogGenericError(Strings.FormatWarningUnknownValuePleaseReport(nameof(announceResponse), announceResponse.Content?.Result));
			...
			if (announceResponse == null) {
				// We've waited long enough, something is definitely wrong with us or the backend
				Bot.TickrLogger.LogGenericWarning(Strings.FormatWarningFailedWithError(nameof(announceResponse)));
			...
			Bot.TickrLogger.LogGenericWarning(Strings.WarningFailed);
		} finally {
	...
	private async Task<bool?> IsEligibleForListing() {
		...
		if (hasPublicInventory != true) {
			Bot.TickrLogger.LogGenericTrace(Strings.FormatWarningFailedWithError($"{nameof(Bot.HasPublicInventory)}: {hasPublicInventory?.ToString() ?? "null"}"));
	...
	private async Task<bool?> IsEligibleForMatching() {
		...
		if (Bot.IsAccountLimited) {
			Bot.TickrLogger.LogGenericTrace(Strings.FormatWarningFailedWithError($"{nameof(Bot.IsAccountLimited)}: {Bot.IsAccountLimited}"));
		...
		if (Bot.IsAccountLocked) {
			Bot.TickrLogger.LogGenericTrace(Strings.FormatWarningFailedWithError($"{nameof(Bot.IsAccountLocked)}: {Bot.IsAccountLocked}"));

			return false;
		}

		// Bot must have Tickr 2FA
		if (!Bot.HasMobileAuthenticator) {
			Bot.TickrLogger.LogGenericTrace(Strings.FormatWarningFailedWithError($"{nameof(Bot.HasMobileAuthenticator)}: {Bot.HasMobileAuthenticator}"));
		...
		if (Bot.BotConfig.MatchableTypes.IsEmpty || Bot.BotConfig.MatchableTypes.All(static type => !AcceptedMatchableTypes.Contains(type))) {
			Bot.TickrLogger.LogGenericTrace(Strings.FormatWarningFailedWithError($"{nameof(Bot.BotConfig.MatchableTypes)}: {string.Join(", ", Bot.BotConfig.MatchableTypes)}"));

			return false;
		}

		// Bot must pass some general trading requirements
		CCredentials_GetSteamGuardDetails_Response? steamGuardStatus = await Bot.TickrHandler.GetSteamGuardStatus().ConfigureAwait(false);

		if (steamGuardStatus == null) {
			Bot.TickrLogger.LogGenericTrace(Strings.FormatWarningFailedWithError($"{nameof(steamGuardStatus)}: null"));
		...
		if (!steamGuardStatus.is_steamguard_enabled || ((steamGuardStatus.timestamp_steamguard_enabled > 0) && ((DateTimeOffset.UtcNow - DateTimeOffset.FromUnixTimeSeconds(steamGuardStatus.timestamp_steamguard_enabled)).TotalDays < MinimumSteamGuardEnabledDays))) {
			Bot.TickrLogger.LogGenericTrace(Strings.FormatWarningFailedWithError($"{nameof(steamGuardStatus.is_steamguard_enabled)}/{nameof(steamGuardStatus.timestamp_steamguard_enabled)}: {steamGuardStatus.is_steamguard_enabled}/{steamGuardStatus.timestamp_steamguard_enabled}"));
		...
		if (!steamGuardStatus.is_twofactor_enabled) {
			Bot.TickrLogger.LogGenericTrace(Strings.FormatWarningFailedWithError($"{nameof(steamGuardStatus.is_twofactor_enabled)}: false"));

			return false;
		}

		CCredentials_LastCredentialChangeTime_Response? credentialChangeTimeDetails = await Bot.TickrHandler.GetCredentialChangeTimeDetails().ConfigureAwait(false);

		if (credentialChangeTimeDetails == null) {
			Bot.TickrLogger.LogGenericTrace(Strings.FormatWarningFailedWithError($"{nameof(credentialChangeTimeDetails)}: null"));
		...
		if ((credentialChangeTimeDetails.timestamp_last_password_reset > 0) && ((DateTimeOffset.UtcNow - DateTimeOffset.FromUnixTimeSeconds(credentialChangeTimeDetails.timestamp_last_password_reset)).TotalDays < MinimumPasswordResetCooldownDays)) {
			Bot.TickrLogger.LogGenericTrace(Strings.FormatWarningFailedWithError($"{nameof(credentialChangeTimeDetails.timestamp_last_password_reset)}: {credentialChangeTimeDetails.timestamp_last_password_reset}"));
	...
	private async void MatchActively(object? state = null) {
		if (TickrApp.GlobalConfig == null) {
			throw new InvalidOperationException(nameof(TickrApp.GlobalConfig));
		}

		if (!TickrApp.GlobalConfig.LicenseID.HasValue || (TickrApp.GlobalConfig.LicenseID == Guid.Empty)) {
			throw new InvalidOperationException(nameof(TickrApp.GlobalConfig.LicenseID));
		}

		if (!Bot.IsConnectedAndLoggedOn || Bot.BotConfig.TradingPreferences.HasFlag(BotConfig.ETradingPreferences.MatchEverything)) {
			Bot.TickrLogger.LogGenericTrace(Strings.ErrorAborted);
		...
		if (eligible != true) {
			Bot.TickrLogger.LogGenericWarning(Strings.FormatWarningFailedWithError($"{nameof(IsEligibleForMatching)}: {eligible?.ToString() ?? "null"}"));
		...
		try {
			Bot.TickrLogger.LogGenericInfo(Strings.Starting);

			HttpStatusCode? licenseStatus = await Backend.GetLicenseStatus(TickrApp.GlobalConfig.LicenseID.Value, WebBrowser).ConfigureAwait(false);

			if (licenseStatus == null) {
				Bot.TickrLogger.LogGenericWarning(Strings.FormatWarningFailedWithError(nameof(licenseStatus)));

				return;
			}

			if (!licenseStatus.Value.IsSuccessCode()) {
				Bot.TickrLogger.LogGenericWarning(Strings.FormatWarningFailedWithError(licenseStatus.Value));
			...
			try {
				assetsForMatching = await Bot.TickrHandler.GetMyInventoryAsync().Where(item => item is { AssetID: > 0, Amount: > 0, ClassID: > 0, RealAppID: > 0, Type: > EAssetType.Unknown, Rarity: > EAssetRarity.Unknown, IsSteamPointsShopItem: false } && acceptedMatchableTypes.Contains(item.Type) && !Bot.BotDatabase.MatchActivelyBlacklistAppIDs.Contains(item.RealAppID)).ToHashSetAsync().ConfigureAwait(false);
			} catch (TimeoutException e) {
				Bot.TickrLogger.LogGenericWarningException(e);
				Bot.TickrLogger.LogGenericWarning(Strings.FormatWarningFailedWithError(nameof(assetsForMatching)));
			...
			if (assetsForMatching.Count == 0) {
				Bot.TickrLogger.LogGenericInfo(Strings.FormatErrorIsEmpty(nameof(assetsForMatching)));
			...
			if (assetsForMatching.Count > MaxItemsCount) {
				Bot.TickrLogger.LogGenericWarning(Strings.FormatWarningFailedWithError($"{nameof(assetsForMatching)} > {MaxItemsCount}"));

				return;
			}

			(HttpStatusCode StatusCode, ImmutableHashSet<ListedUser> Users)? response = await Backend.GetListedUsersForMatching(TickrApp.GlobalConfig.LicenseID.Value, Bot, WebBrowser, assetsForMatching, acceptedMatchableTypes).ConfigureAwait(false);

			if (response == null) {
				Bot.TickrLogger.LogGenericWarning(Strings.FormatWarningFailedWithError(nameof(response)));

				return;
			}

			if (!response.Value.StatusCode.IsSuccessCode()) {
				Bot.TickrLogger.LogGenericWarning(Strings.FormatWarningFailedWithError(response.Value.StatusCode));

				return;
			}

			if (response.Value.Users.IsEmpty) {
				Bot.TickrLogger.LogGenericInfo(Strings.FormatErrorIsEmpty(nameof(response.Value.Users)));
			...
			Bot.TickrLogger.LogGenericInfo(Strings.Done);
		} finally {
	...
	private async Task<bool> MatchActively(ImmutableHashSet<ListedUser> listedUsers, HashSet<Asset> ourAssets, HashSet<EAssetType> acceptedMatchableTypes) {
		...
		if (MatchingUtilities.IsEmptyForMatching(ourFullState, ourTradableState)) {
			...
			Bot.TickrLogger.LogGenericTrace(Strings.FormatErrorIsEmpty($"{nameof(ourFullState)} || {nameof(ourTradableState)}"));
		...
		if (matchActivelyTradeOfferIDs.Count > 0) {
			...
			HashSet<TradeOffer>? sentTradeOffers = await Bot.TickrWebHandler.GetTradeOffers(true, false, true, false).ConfigureAwait(false);
			...
			if (sentTradeOffers != null) {
				...
				foreach (TradeOffer tradeOffer in sentTradeOffers.Where(tradeOffer => (tradeOffer.State == ETradeOfferState.Active) && matchActivelyTradeOfferIDs.Contains(tradeOffer.TradeOfferID))) {
					...
					if (!await Bot.TickrWebHandler.CancelTradeOffer(tradeOffer.TradeOfferID).ConfigureAwait(false)) {
		...
		byte maxTradeHoldDuration = TickrApp.GlobalConfig?.MaxTradeHoldDuration ?? GlobalConfig.DefaultMaxTradeHoldDuration;
		...
		foreach (ListedUser listedUser in listedUsers.Where(listedUser => (listedUser.SteamID != Bot.SteamID) && acceptedMatchableTypes.Overlaps(listedUser.MatchableTypes) && !Bot.IsBlacklistedFromTrades(listedUser.SteamID)).OrderByDescending(listedUser => !deprioritizedSteamIDs.Contains(listedUser.SteamID)).ThenByDescending(static listedUser => listedUser.TotalGamesCount > 1).ThenByDescending(static listedUser => listedUser.MatchEverything).ThenBy(static listedUser => listedUser.TotalInventoryCount)) {
			if (failuresInRow >= WebBrowser.MaxTries) {
				Bot.TickrLogger.LogGenericWarning(Strings.FormatWarningFailedWithError($"{nameof(failuresInRow)} >= {WebBrowser.MaxTries}"));

				break;
			}

			if (!Bot.IsConnectedAndLoggedOn) {
				Bot.TickrLogger.LogGenericWarning(Strings.BotNotConnected);
			...
			Bot.TickrLogger.LogGenericTrace($"{listedUser.SteamID}...");
			...
			for (byte i = 0; i < Trading.MaxTradesPerAccount; i++) {
						...
						foreach ((ulong ourItem, uint ourFullAmount) in ourFullSet.Where(static item => item.Value > 1).OrderByDescending(static item => item.Value)) {
							...
							foreach ((ulong theirItem, uint theirTradableAmount) in theirTradableItems.OrderBy(item => ourFullSet.GetValueOrDefault(item.Key))) {
								...
								if (tradeHoldDuration == null) {
									tradeHoldDuration = await Bot.TickrWebHandler.GetCombinedTradeHoldDurationAgainstUser(listedUser.SteamID, listedUser.TradeToken).ConfigureAwait(false);
									...
									if (tradeHoldDuration == null) {
										...
										Bot.TickrLogger.LogGenericTrace(Strings.FormatErrorIsEmpty(nameof(tradeHoldDuration)));
									...
									if ((tradeHoldDuration.Value > maxTradeHoldDuration) || (tradeHoldDuration.Value > listedUser.MaxTradeHoldDuration)) {
										...
										Bot.TickrLogger.LogGenericTrace($"{tradeHoldDuration.Value} > {maxTradeHoldDuration} || {listedUser.MaxTradeHoldDuration}");
				...
				if (skippedSetsThisTrade.Count == 0) {
					Bot.TickrLogger.LogGenericTrace(Strings.FormatErrorIsEmpty(nameof(skippedSetsThisTrade)));
				...
				Bot.TickrLogger.LogGenericInfo(Localization.Strings.FormatMatchingFound(itemsToReceive.Count, listedUser.SteamID, listedUser.Nickname));

				Bot.TickrLogger.LogGenericTrace($"{Bot.SteamID} <- {string.Join(", ", itemsToReceive.Select(static item => $"{item.RealAppID}/{item.Type}/{item.Rarity}/{item.ClassID} #{item.Amount}"))} | {string.Join(", ", itemsToGive.Select(static item => $"{item.RealAppID}/{item.Type}/{item.Rarity}/{item.ClassID} #{item.Amount}"))} -> {listedUser.SteamID}");

				(bool success, HashSet<ulong>? tradeOfferIDs, HashSet<ulong>? mobileTradeOfferIDs) = await Bot.TickrWebHandler.SendTradeOffer(listedUser.SteamID, itemsToGive, itemsToReceive, listedUser.TradeToken, nameof(MatchActively), true).ConfigureAwait(false);
				...
				if (mobileTradeOfferIDs?.Count > 0) {
					...
					if (pendingMobileTradeOfferIDs.Count >= MaxTradeOffersActive) {
						...
						if (!twoFactorSuccess) {
							Bot.TickrLogger.LogGenericWarning(Localization.Strings.FormatActivelyMatchingSomeConfirmationsFailed(handledConfirmations?.Count ?? 0, pendingMobileTradeOfferIDs.Count));
						}
				...
				if (!success) {
					...
					Bot.TickrLogger.LogGenericWarning(Localization.Strings.FormatTradeOfferFailed(listedUser.SteamID, listedUser.Nickname));
		...
		Bot.TickrLogger.LogGenericInfo(Localization.Strings.FormatActivelyMatchingItemsRound(matchedSets));
	...
	private async void OnHeartBeatTimer(object? state = null) {
		...
		try {
			...
			if (response.StatusCode.IsRedirectionCode()) {
				...
				Bot.TickrLogger.LogGenericWarning(Strings.FormatWarningFailedWithError(response.StatusCode));

				if (response.FinalUri.Host != TickrWebHandler.SteamCommunityURL.Host) {
					TickrApp.TickrLogger.LogGenericError(Strings.FormatWarningUnknownValuePleaseReport(nameof(response.FinalUri), response.FinalUri));
			...
			if (!response.StatusCode.IsSuccessCode()) {
				...
				switch (response.StatusCode) {
					case HttpStatusCode.Conflict:
						// TickrNet told us to that we need to announce again
