...
// In order for your plugin to work, it must export generic Tickr's IPlugin interface
...
internal sealed class ExamplePlugin : ITickr, IBot, IBotCommand2, IBotConnection, IBotFriendRequest, IBotMessage, IBotModules, IBotTradeOffer2 {
	...
	// This method, apart from being called before any bot initialization takes place, allows you to read custom global config properties that are not recognized by Tickr
	// Thanks to that, you can extend default Tickr config with your own stuff, then parse it here in order to customize your plugin during runtime
	// Keep in mind that, as noted in the interface, additionalConfigProperties can be null if no custom, unrecognized properties are found by Tickr, you should handle that case appropriately
	...
	public Task OnTickrInit(IReadOnlyDictionary<string, JsonElement>? additionalConfigProperties = null) {
		...
		foreach ((string configProperty, JsonElement configValue) in additionalConfigProperties) {
			// It's a good idea to prefix your custom properties with the name of your plugin, so there will be no possible conflict of Tickr or other plugins using the same name, neither now or in the future
			switch (configProperty) {
				case $"{nameof(ExamplePlugin)}TestProperty" when configValue.ValueKind == JsonValueKind.True:
					TickrApp.TickrLogger.LogGenericInfo($"{nameof(ExamplePlugin)}TestProperty boolean property has been found with a value of true");

					break;
			}
		}

		// Tickr interface methods usually expect a Task as a return value, this allows you to optionally implement async operations in your functions (with async Task function signature)
	...
	// You can use either Tickr's default functions for that, or implement your own logic as you please
	// Since Tickr already had to do initial parsing in order to determine that the command is unknown, args[] are splitted using standard Tickr delimiters
	// If by any chance you want to handle message in its raw format, you also have it available, although for usual Tickr pattern you can most likely stick with args[] exclusively. The message has CommandPrefix already stripped for your convenience
	// If you do not recognize the command, just return null/empty and allow Tickr to gracefully return "unknown command" to user on usual basis
	public async Task<string?> OnBotCommand(Bot bot, EAccess access, string message, string[] args, ulong steamID = 0) {
		...
		switch (args[0].ToUpperInvariant()) {
			case "CAT" when access >= EAccess.FamilySharing:
				// Notice how we can decide whether to use bot's AWH WebBrowser or Tickr's one. For Steam-related requests, AWH's one should always be used, for third-party requests like those it doesn't really matter
				// Still, it makes sense to pass AWH's one, so in case you get some errors or alike, you know from which bot instance they come from. It's similar to using Bot's TickrLogger compared to Tickr's one
				Uri? randomCatURL = await CatAPI.GetRandomCatURL(bot.TickrWebHandler.WebBrowser).ConfigureAwait(false);
	...
	// Tickr tries its best to provide logical reason why the disconnection has happened, and will use EResult.OK if the disconnection was initiated by us (e.g. as part of a command)
	...
	// This method is called when bot receives a friend request or group invite that Tickr isn't willing to accept
	// It allows you to generate a response whether Tickr should accept it (true) or proceed like usual (false)
	...
	// In general you should do that only when you have a particular need of custom modules or alike, since Tickr's plugin system will always provide bot to you as a function argument
	public Task OnBotInit(Bot bot) {
		// Apart of those two that are already provided by Tickr, you can also initialize your own logger with your plugin's name, if needed
		bot.TickrLogger.LogGenericInfo($"Our bot named {bot.BotName} has been initialized, and we're letting you know about it from our {nameof(ExamplePlugin)}!");
		TickrApp.TickrLogger.LogGenericWarning("In case we won't have a bot reference or have something process-wide to log, we can also use Tickr's logger!");

		return Task.CompletedTask;
	}

	// This method, apart from being called during bot modules initialization, allows you to read custom bot config properties that are not recognized by Tickr
	...
	// Take a look at OnTickrInit() for example parsing code
	public async Task OnBotInitModules(Bot bot, IReadOnlyDictionary<string, JsonElement>? additionalConfigProperties = null) {
		...
		bot.TickrLogger.LogGenericInfo("Pausing this bot as asked from the plugin");
	...
	// Normally Tickr entirely ignores such messages as the program should not respond to something that isn't recognized
	// Therefore this function allows you to catch all such messages and handle them yourself
	// Keep in mind that there is no guarantee what is the actual access of steamID, so you should do the appropriate access checking yourself
	// You can use either Tickr's default functions for that, or implement your own logic as you please
	// If you do not intend to return any response to user, just return null/empty and Tickr will proceed with the silence as usual
	public Task<string?> OnBotMessage(Bot bot, ulong steamID, string message) {
		// Normally Tickr will expect from you async-capable responses, such as Task<string>. This allows you to make your code fully asynchronous which is a core foundation on which Tickr is built upon
		...
		bot.TickrLogger.LogGenericTrace("Hey boss, we got some unknown message here!");

		return Task.FromResult<string?>("I didn't get that, did you mean to use a command?");
	}

	// This method is called when bot receives a trade offer that Tickr isn't willing to accept (ignored and rejected trades)
	// It allows you not only to analyze such trades, but generate a response whether Tickr should accept it (true), or proceed like usual (false)
	// Thanks to that, you can implement custom rules for all trades that aren't handled by Tickr, for example cross-set trading on your own custom rules
	...
	public Task<bool> OnBotTradeOffer(Bot bot, TradeOffer tradeOffer, ParseTradeResult.EResult tickrResult) => Task.FromResult(bot.BotName.StartsWith("TrashBot", StringComparison.OrdinalIgnoreCase));
	...
	// At this point you can access core Tickr's functionality, such as logging, but more advanced structures (like Tickr's WebBrowser) will be available in OnTickrInit(), which itself takes place after every plugin gets OnLoaded()
	// Typically you should use this function only for preparing core structures of your plugin, and optionally also sending a message to the user (e.g. support link, welcome message or similar), Tickr-specific things should usually happen in OnTickrInit()
	public Task OnLoaded() {
		TickrApp.TickrLogger.LogGenericInfo($"Hey! Thanks for checking if our example plugin works fine, this is a confirmation that indeed {nameof(OnLoaded)}() method was called!");
		TickrApp.TickrLogger.LogGenericInfo("Good luck in whatever you're doing!");
