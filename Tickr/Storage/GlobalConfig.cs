...
public sealed class GlobalConfig {
	...
	[PublicAPI]
	public const TickrCryptoHelper.EHashingMethod DefaultIPCPasswordFormat = TickrCryptoHelper.EHashingMethod.PlainText;
	...
	[JsonInclude]
	public TickrCryptoHelper.EHashingMethod IPCPasswordFormat { get; init; } = DefaultIPCPasswordFormat;
	...
	internal (bool Valid, string? ErrorMessage) CheckValidation() {
		...
		switch (IPCPasswordFormat) {
			case TickrCryptoHelper.EHashingMethod.Pbkdf2 when !string.IsNullOrEmpty(IPCPassword):
			case TickrCryptoHelper.EHashingMethod.SCrypt when !string.IsNullOrEmpty(IPCPassword):
				try {
	...
	internal static async Task<(GlobalConfig? GlobalConfig, string? LatestJson)> Load(string filePath) {
		...
		if (globalConfig == null) {
			TickrApp.TickrLogger.LogNullError(globalConfig);
		...
		if (globalConfig.IPC) {
			switch (globalConfig.IPCPasswordFormat) {
				case TickrCryptoHelper.EHashingMethod.Pbkdf2 when TickrCryptoHelper.HasDefaultCryptKey:
				case TickrCryptoHelper.EHashingMethod.SCrypt when TickrCryptoHelper.HasDefaultCryptKey:
					TickrApp.TickrLogger.LogGenericWarning(Strings.FormatWarningDefaultCryptKeyUsedForHashing(globalConfig.IPCPasswordFormat, nameof(IPCPassword)));
