...
public interface ISteamPICSChanges : IPlugin {
	...
	/// <summary>
	///     Tickr will call this method when it'll be necessary to restart the history of PICS changes. This can happen due to Steam limitation in which we're unable to keep history going if we're too far behind (approx 5k changeNumbers). If you're relying on continuous history of app/package PICS changes sent by <see cref="OnPICSChanges" />, Tickr can no longer guarantee that upon calling this method, therefore you should start clean.
	/// </summary>
