...
public static class PluginsCore {
	...
	public static async Task<ICrossProcessSemaphore> GetCrossProcessSemaphore(string objectName) {
		...
		} else if (!string.IsNullOrEmpty(TickrApp.GlobalConfig.WebProxyText)) {
			objectName += $"-{Convert.ToHexString(SHA256.HashData(Encoding.UTF8.GetBytes(TickrApp.GlobalConfig.WebProxyText)))}";
		}
	...
	internal static async Task<uint> GetChangeNumberToStartFrom() {
		uint lastChangeNumber = TickrApp.GlobalDatabase?.LastChangeNumber ?? 0;
	...
	internal static async Task<bool> InitPlugins() {
		...
		if ((assemblies == null) || (assemblies.Count == 0)) {
			TickrApp.TickrLogger.LogGenericTrace(Strings.NothingFound);

			return true;
		}

		TickrApp.TickrLogger.LogGenericInfo(Strings.FormatInitializing(nameof(Plugins)));

		foreach (Assembly assembly in assemblies) {
			if (Debugging.IsUserDebugging) {
				TickrApp.TickrLogger.LogGenericDebug(Strings.FormatInitializing(assembly.FullName));
			}
			...
			try {
				...
				TickrApp.TickrLogger.LogGenericError(Strings.FormatWarningFailedWithError(assembly.FullName));
				TickrApp.TickrLogger.LogGenericException(e);
		...
		try {
			...
			TickrApp.TickrLogger.LogGenericError(Strings.FormatWarningFailedWithError(e.TypeName));
			TickrApp.TickrLogger.LogGenericException(e);
		...
		foreach (IPlugin plugin in activePlugins) {
			try {
				TickrApp.TickrLogger.LogGenericInfo(Strings.FormatPluginLoading(plugin.Name, plugin.Version));

				if (!Program.IgnoreUnsupportedEnvironment && plugin is OfficialPlugin officialPlugin && !officialPlugin.HasSameVersion()) {
					TickrApp.TickrLogger.LogGenericError(Strings.FormatWarningUnsupportedOfficialPlugins(plugin.Name, plugin.Version, SharedInfo.AssemblyVersion));
				...
				TickrApp.TickrLogger.LogGenericInfo(Strings.FormatPluginLoaded(plugin.Name));
			} catch (Exception e) {
				TickrApp.TickrLogger.LogGenericException(e);
		...
		if (HasCustomPluginsLoaded) {
			TickrApp.TickrLogger.LogGenericInfo(Strings.PluginsWarning);
		...
		GlobalConfig.EPluginsUpdateMode pluginsUpdateMode = TickrApp.GlobalConfig?.PluginsUpdateMode ?? GlobalConfig.DefaultPluginsUpdateMode;
		ImmutableHashSet<string> pluginsUpdateList = TickrApp.GlobalConfig?.PluginsUpdateList ?? GlobalConfig.DefaultPluginsUpdateList;
		...
		foreach (IPluginUpdates plugin in activePlugins.OfType<IPluginUpdates>()) {
			...
			if (string.IsNullOrEmpty(pluginAssemblyName)) {
				TickrApp.TickrLogger.LogNullError(nameof(pluginAssemblyName));
			...
			switch (pluginsUpdateMode) {
				...
				case GlobalConfig.EPluginsUpdateMode.Whitelist when pluginsUpdateList.Contains(pluginAssemblyName):
					...
					TickrApp.TickrLogger.LogGenericInfo(Strings.FormatPluginUpdateEnabled(plugin.Name, pluginAssemblyName));

					break;
				case GlobalConfig.EPluginsUpdateMode.Blacklist when pluginsUpdateList.Contains(pluginAssemblyName):
				case GlobalConfig.EPluginsUpdateMode.Whitelist when !pluginsUpdateList.Contains(pluginAssemblyName):
					TickrApp.TickrLogger.LogGenericInfo(Strings.FormatPluginUpdateDisabled(plugin.Name, pluginAssemblyName));
		...
		if (activePluginUpdates.Count > 0) {
			if (activePluginUpdates.Any(static plugin => plugin is not OfficialPlugin)) {
				TickrApp.TickrLogger.LogGenericWarning(Strings.CustomPluginUpdatesEnabled);
			}
	...
	internal static async Task OnTickrInitModules(IReadOnlyDictionary<string, JsonElement>? additionalConfigProperties = null) {
		if (ActivePlugins.Count == 0) {
			return;
		}

		try {
			await Utilities.InParallel(ActivePlugins.OfType<ITickr>().Select(plugin => plugin.OnTickrInit(additionalConfigProperties))).ConfigureAwait(false);
		} catch (Exception e) {
			TickrApp.TickrLogger.LogGenericException(e);
		}
	}
	...
	internal static async Task<bool> OnBotTradeOffer(Bot bot, TradeOffer tradeOffer, ParseTradeResult.EResult tickrResult) {
		ArgumentNullException.ThrowIfNull(bot);
		...
		try {
			responses = await Utilities.InParallel(ActivePlugins.OfType<IBotTradeOffer2>().Select(plugin => plugin.OnBotTradeOffer(bot, tradeOffer, tickrResult))).ConfigureAwait(false);
		} catch (Exception e) {
			TickrApp.TickrLogger.LogGenericException(e);
	...
	internal static async Task<bool> UpdatePlugins(Version tickrVersion, bool tickrUpdate, GlobalConfig.EUpdateChannel? updateChannel = null, bool updateOverride = false, bool forced = false) {
		...
		return await UpdatePlugins(tickrVersion, tickrUpdate, ActivePluginUpdates, updateChannel, updateOverride, forced).ConfigureAwait(false);
	}

	internal static async Task<bool> UpdatePlugins(Version tickrVersion, bool tickrUpdate, IReadOnlyCollection<IPluginUpdates> plugins, GlobalConfig.EUpdateChannel? updateChannel = null, bool updateOverride = false, bool forced = false) {
		ArgumentNullException.ThrowIfNull(tickrVersion);
		...
		updateChannel ??= TickrApp.GlobalConfig?.UpdateChannel ?? GlobalConfig.DefaultUpdateChannel;

		if (updateChannel == GlobalConfig.EUpdateChannel.None) {
			return false;
		}

		TickrApp.TickrLogger.LogGenericInfo(Strings.PluginUpdatesChecking);

		IList<bool> pluginUpdates = await Utilities.InParallel(plugins.Select(plugin => UpdatePlugin(tickrVersion, tickrUpdate, plugin, updateChannel.Value, updateOverride, forced))).ConfigureAwait(false);
	...
	private static HashSet<Assembly>? LoadAssembliesFrom(string path) {
		...
		try {
			foreach (string assemblyPath in Directory.EnumerateFiles(path, "*.dll", SearchOption.AllDirectories)) {
				...
				if (skip) {
					TickrApp.TickrLogger.LogGenericTrace(Strings.FormatWarningSkipping(assemblyPath));
				...
				try {
					...
					TickrApp.TickrLogger.LogGenericError(Strings.FormatErrorIsInvalid(assemblyPath));
	...
	private static async Task<bool> UpdatePlugin(Version tickrVersion, bool tickrUpdate, IPluginUpdates plugin, GlobalConfig.EUpdateChannel updateChannel, bool updateOverride, bool forced) {
		ArgumentNullException.ThrowIfNull(tickrVersion);
		...
		try {
			...
			TickrApp.TickrLogger.LogGenericInfo(Strings.FormatPluginUpdateChecking(pluginName));
			...
			Uri? releaseURL = await plugin.GetTargetReleaseURL(tickrVersion, BuildInfo.Variant, tickrUpdate, updateChannel, forced).ConfigureAwait(false);

			if (releaseURL == null) {
				return false;
			}

			if (!updateOverride && ((TickrApp.GlobalConfig?.UpdatePeriod ?? GlobalConfig.DefaultUpdatePeriod) == 0)) {
				TickrApp.TickrLogger.LogGenericInfo(Strings.FormatPluginUpdateNewVersionAvailable(pluginName));

				return false;
			}

			TickrApp.TickrLogger.LogGenericInfo(Strings.FormatPluginUpdateInProgress(pluginName));
			...
			try {
				response = await TickrApp.WebBrowser.UrlGetToBinary(releaseURL, progressReporter: progressReporter).ConfigureAwait(false);
			} finally {
				progressReporter.ProgressChanged -= onProgressChanged;
			}
			...
			TickrApp.TickrLogger.LogGenericInfo(Strings.PatchingFiles);
			...
			await using (memoryStream.ConfigureAwait(false)) {
				...
				await using (zipArchive.ConfigureAwait(false)) {
					...
					if (!await Utilities.UpdateFromArchive(zipArchive, assemblyDirectory).ConfigureAwait(false)) {
						TickrApp.TickrLogger.LogGenericError(Strings.WarningFailed);
		...
		TickrApp.TickrLogger.LogGenericInfo(Strings.FormatPluginUpdateFinished(pluginName));
