...
public static class SharedInfo {
	...
	internal const string Tickr = nameof(Tickr);
	internal const ulong TickrGroupSteamID = 0;
	internal const string AssemblyName = nameof(Tickr);
	internal const string DatabaseExtension = ".db";
	internal const string DebugDirectory = "debug";
	internal const string DefaultPluginTemplateGithubRepo = "8owner8/PluginTemplate";
	internal const string EnvironmentVariableArguments = $"{Tickr}_ARGS";
	internal const string EnvironmentVariableCryptKey = $"{Tickr}_CRYPTKEY";
	internal const string EnvironmentVariableCryptKeyFile = $"{EnvironmentVariableCryptKey}_FILE";
	internal const string EnvironmentVariableNetworkGroup = $"{Tickr}_NETWORK_GROUP";
	internal const string EnvironmentVariablePath = $"{Tickr}_PATH";
	internal const string GithubRepo = $"8owner8/{AssemblyName}";
	internal const string GlobalConfigFileName = $"{Tickr}{JsonConfigExtension}";
	internal const string GlobalCrashFileName = $"{Tickr}.crash";
	internal const string GlobalDatabaseFileName = $"{Tickr}{DatabaseExtension}";
	...
	internal static Version AssemblyVersion { get; } = Assembly.GetExecutingAssembly().GetName().Version ?? throw new InvalidOperationException(nameof(AssemblyVersion));
	internal static Version Version => VersionCache ??= LoadVersion();

	private static Version? VersionCache;

	// The assembly metadata is frozen at a fixed version (see Directory.Build.props) so satellite/resource
	// assemblies stay byte-identical between releases - otherwise every version bump churns ~160 resource
	// DLLs into the update delta. The real product version is stamped by the launcher into version.txt.
	private static Version LoadVersion() {
		try {
			string versionFile = Path.Combine(HomeDirectory, "version.txt");

			if (File.Exists(versionFile) && Version.TryParse(File.ReadAllText(versionFile).Trim().TrimStart('v', 'V'), out Version? version)) {
				return version;
			}
		} catch (Exception e) {
