...
internal static class OS {
	...
	internal static void Init(GlobalConfig.EOptimizationMode optimizationMode) {
		...
		switch (optimizationMode) {
			case GlobalConfig.EOptimizationMode.MaxPerformance:
				// No specific tuning required for now, Tickr is optimized for max performance by default
				break;
	...
	internal static bool VerifyEnvironment() {
		...
		TickrApp.TickrLogger.LogGenericError(Strings.FormatWarningUnknownValuePleaseReport(nameof(BuildInfo.Variant), BuildInfo.Variant));
	...
	private static async Task LinuxKeepSystemActive() {
		...
		if (string.IsNullOrEmpty(systemAddress)) {
			TickrApp.TickrLogger.LogGenericError(Strings.FormatWarningFailedWithError(nameof(systemAddress)));
		...
		try {
			...
			TickrApp.TickrLogger.LogGenericWarning(Strings.WarningNoSystemRequiredLinuxDependencies);
	...
	private static void WindowsKeepSystemActive() {
		...
		// If user wishes to enter sleep mode, then they should use ShutdownOnFarmingFinished or manage the Tickr process with third-party tool or script
		...
		if (result == NativeMethods.EExecutionState.None) {
			TickrApp.TickrLogger.LogGenericError(Strings.FormatWarningFailedWithError(result));
		}
	}
}
