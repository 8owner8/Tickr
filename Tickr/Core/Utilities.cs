...
public static class Utilities {
	...
	internal static void OnProgressChanged(string fileName, byte progressPercentage) {
		...
		TickrApp.TickrLogger.LogGenericDebug($"{fileName} {progressPercentage}%...");
	}
		...
		try {
			...
			if (Directory.Exists(updateDirectory)) {
				if (!updateCleanup) {
					...
					TickrApp.TickrLogger.LogGenericInfo(Strings.UpdateCleanup);
				}
	...
	internal static void WarnAboutIncompleteTranslation(ResourceManager resourceManager) {
		...
		if (defaultResourceSet == null) {
			TickrApp.TickrLogger.LogNullError(defaultResourceSet);
		...
		if (currentResourceSet == null) {
			TickrApp.TickrLogger.LogNullError(currentResourceSet);
		...
		if (currentStringObjects.Count < defaultStringObjects.Count) {
			...
			TickrApp.TickrLogger.LogGenericInfo(Strings.FormatTranslationIncomplete($"{CultureInfo.CurrentUICulture.Name} ({CultureInfo.CurrentUICulture.EnglishName})", translationCompleteness.ToString("P1", CultureInfo.CurrentCulture)));
		}
	}
