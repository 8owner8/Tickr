...
public static class TickrApp {
	...
	[PublicAPI]
	public static readonly TickrLogger TickrLogger = new(SharedInfo.Tickr);
	...
	internal static async Task<bool> Init() {
		...
		WebBrowser = new WebBrowser(TickrLogger, GlobalConfig.WebProxy, true);
		...
		if (!Program.IgnoreUnsupportedEnvironment && !await ProtectAgainstCrashes().ConfigureAwait(false)) {
			TickrLogger.LogFatalError(Strings.ErrorTooManyCrashes);
		...
		await PluginsCore.OnTickrInitModules(GlobalConfig.AdditionalProperties).ConfigureAwait(false);
		...
		bool hasInteractiveConsole = false;

		try {
			hasInteractiveConsole = !Program.Service && !GlobalConfig.Headless && !Console.IsInputRedirected;
		} catch {
			// Ignored in GUI / WinExe mode
		}
		...
		if (GlobalConfig.IPC) {
			await TickrKestrel.Start().ConfigureAwait(false);
		}
	...
	internal static bool IsValidBotName(string botName) {
		...
		if (botName.Equals(SharedInfo.Tickr, StringComparison.OrdinalIgnoreCase)) {
			return false;
		}
	...
	internal static async Task RestartOrExit() {
		...
		if (Program.RestartAllowed && GlobalConfig.AutoRestart) {
			TickrLogger.LogGenericInfo(Strings.Restarting);
			await Task.Delay(SharedInfo.ShortInformationDelay).ConfigureAwait(false);
			await Program.Restart().ConfigureAwait(false);
		} else {
			TickrLogger.LogGenericInfo(Strings.Exiting);
			await Task.Delay(SharedInfo.ShortInformationDelay).ConfigureAwait(false);
	...
	internal static async Task<(bool Updated, Version? NewVersion)> Update(GlobalConfig.EUpdateChannel? updateChannel = null, bool updateOverride = false, bool forced = false) {
		...
		(bool updated, Version? newVersion) = await UpdateTickr(updateChannel, updateOverride, forced).ConfigureAwait(false);

		if (!updated) {
			// Tickr wasn't updated as part of the process, update the plugins alone
	...
	private static void InitConfigWatchEvents() {
		...
		if (Bot.BotsComparer == null) {
			// This should never happen, as BotsComparer is always initialized via Bot.Init() in Init(), but let's protect ourselves from a fatal crash on invalid states
			TickrLogger.LogGenericError(Strings.FormatErrorObjectIsNull(nameof(Bot.BotsComparer)));

			Bot.Init(StringComparer.Ordinal);
		}
	...
	private static async Task InitRateLimiters() {
		...
		WebLimitingSemaphores ??= new Dictionary<Uri, (ICrossProcessSemaphore RateLimitingSemaphore, SemaphoreSlim OpenConnectionsSemaphore)>(5) {
			{ TickrWebHandler.SteamCheckoutURL, (await PluginsCore.GetCrossProcessSemaphore($"{nameof(TickrWebHandler)}-{nameof(TickrWebHandler.SteamCheckoutURL)}").ConfigureAwait(false), new SemaphoreSlim(WebBrowser.MaxConnections, WebBrowser.MaxConnections)) },
			{ TickrWebHandler.SteamCommunityURL, (await PluginsCore.GetCrossProcessSemaphore($"{nameof(TickrWebHandler)}-{nameof(TickrWebHandler.SteamCommunityURL)}").ConfigureAwait(false), new SemaphoreSlim(WebBrowser.MaxConnections, WebBrowser.MaxConnections)) },
			{ TickrWebHandler.SteamHelpURL, (await PluginsCore.GetCrossProcessSemaphore($"{nameof(TickrWebHandler)}-{nameof(TickrWebHandler.SteamHelpURL)}").ConfigureAwait(false), new SemaphoreSlim(WebBrowser.MaxConnections, WebBrowser.MaxConnections)) },
			{ TickrWebHandler.SteamStoreURL, (await PluginsCore.GetCrossProcessSemaphore($"{nameof(TickrWebHandler)}-{nameof(TickrWebHandler.SteamStoreURL)}").ConfigureAwait(false), new SemaphoreSlim(WebBrowser.MaxConnections, WebBrowser.MaxConnections)) },
			{ WebAPI.DefaultBaseAddress, (await PluginsCore.GetCrossProcessSemaphore($"{nameof(TickrWebHandler)}-{nameof(WebAPI)}").ConfigureAwait(false), new SemaphoreSlim(WebBrowser.MaxConnections, WebBrowser.MaxConnections)) }
		}.ToFrozenDictionary();
	}
	...
	private static async Task OnChangedConfigFile(string name) {
		...
		TickrLogger.LogGenericInfo(Strings.IPCConfigChanged);

		await TickrKestrel.Restart().ConfigureAwait(false);
	}
	...
	private static async Task OnConfigChanged() {
		...
		if (globalConfig == null) {
			// Invalid config file, we allow user to fix it without destroying the Tickr instance right away
			return;
		}
		...
		TickrLogger.LogGenericInfo(Strings.GlobalConfigChanged);
	...
	private static async Task OnCreatedJsonFile(string name, string fullPath) {
		...
		if (Bot.Bots.TryGetValue(botName, out Bot? bot)) {
			...
			if (Bot.Bots.Count > MaximumRecommendedBotsCount) {
				TickrLogger.LogGenericWarning(Strings.FormatWarningExcessiveBotsCount(MaximumRecommendedBotsCount));
			}
		}
	}
	...
	private static async Task OnDeletedJsonConfigFile(string name, string fullPath) {
		...
		if (botName.Equals(SharedInfo.Tickr, StringComparison.OrdinalIgnoreCase)) {
			...
			TickrLogger.LogGenericError(Strings.ErrorGlobalConfigRemoved);
	...
	private static async Task RegisterBots() {
		...
		switch (botNames.Count) {
			case 0:
				TickrLogger.LogGenericWarning(Strings.ErrorNoBotsDefined);
	...
	private static async Task UpdateAndRestart() {
		...
		if ((AutoUpdatesTimer == null) && (GlobalConfig.UpdatePeriod > 0)) {
			...
			TickrLogger.LogGenericInfo(Strings.FormatAutoUpdateCheckInfo(autoUpdatePeriod.ToHumanReadable()));
		}
		...
		if (!updated) {
			if ((newVersion != null) && (SharedInfo.Version > newVersion)) {
				...
				TickrLogger.LogGenericWarning(Strings.WarningPreReleaseVersion);
				await Task.Delay(SharedInfo.InformationDelay).ConfigureAwait(false);
			}
	...
	private static async Task<(bool Updated, Version? NewVersion)> UpdateTickr(GlobalConfig.EUpdateChannel? channel = null, bool updateOverride = false, bool forced = false) {
		...
		try {
			...
			TickrLogger.LogGenericInfo(Strings.UpdateCheckingNewVersion);
			...
			if ((releaseResponse == null) || string.IsNullOrEmpty(releaseResponse.Tag)) {
				TickrLogger.LogGenericWarning(Strings.ErrorUpdateCheckFailed);

				return (false, null);
			}

			Version newVersion = new(releaseResponse.Tag.TrimStart('v', 'V'));

			TickrLogger.LogGenericInfo(Strings.FormatUpdateVersionInfo(SharedInfo.Version, newVersion));
			...
			if (!updateOverride && (GlobalConfig.UpdatePeriod == 0)) {
				TickrLogger.LogGenericInfo(Strings.UpdateNewVersionAvailable);
				await Task.Delay(SharedInfo.ShortInformationDelay).ConfigureAwait(false);
			...
			if (releaseResponse.Assets.IsEmpty) {
				TickrLogger.LogGenericWarning(Strings.ErrorUpdateNoAssets);

				return (false, newVersion);
			}

			targetFile = $"{SharedInfo.Tickr}-{BuildInfo.Variant}.zip";
			...
			if (binaryAsset == null) {
				TickrLogger.LogGenericWarning(Strings.ErrorUpdateNoAssetForThisVersion);

				return (false, newVersion);
			}

			TickrLogger.LogGenericInfo(Strings.FetchingChecksumFromRemoteServer);

			// Keep short timeout allowed for this call, as we don't want to hold the flow for too long
			using CancellationTokenSource tickrNetCancellation = new(TimeSpan.FromSeconds(15));

			string? remoteChecksum = await TickrNet.FetchBuildChecksum(newVersion, BuildInfo.Variant, tickrNetCancellation.Token).ConfigureAwait(false);

			switch (remoteChecksum) {
				case null:
					// Timeout or error, refuse to update as a security measure
					TickrLogger.LogGenericWarning(Strings.ChecksumTimeout);

					return (false, newVersion);
				case "":
					// Unknown checksum, release too new or actual malicious build published, no need to scare the user as it's 99.99% the first
					TickrLogger.LogGenericWarning(Strings.ChecksumMissing);

					return (false, newVersion);
			}

			if (!string.IsNullOrEmpty(releaseResponse.ChangelogPlainText)) {
				TickrLogger.LogGenericInfo(releaseResponse.ChangelogPlainText);
			}

			TickrLogger.LogGenericInfo(Strings.FormatUpdateDownloadingNewVersion(newVersion, binaryAsset.Size / 1024 / 1024));
			...
			TickrLogger.LogGenericInfo(Strings.VerifyingChecksumWithRemoteServer);
			...
			if (!checksum.Equals(remoteChecksum, StringComparison.OrdinalIgnoreCase)) {
				TickrLogger.LogGenericError(Strings.ChecksumWrong);
			...
			bool kestrelWasRunning = TickrKestrel.IsRunning;

			if (kestrelWasRunning) {
				// We disable TickrKestrel here as the update process moves the core files and might result in IPC crash
				TickrCoreController.PendingVersionUpdate = newVersion;

				try {
					await TickrKestrel.Stop().ConfigureAwait(false);
				} catch (Exception e) {
					TickrLogger.LogGenericWarningException(e);
				} finally {
					TickrCoreController.PendingVersionUpdate = null;
				}
			}

			TickrLogger.LogGenericInfo(Strings.PatchingFiles);
			...
			try {
				...
				await using (memoryStream.ConfigureAwait(false)) {
					...
					await using (zipArchive.ConfigureAwait(false)) {
						if (!await UpdateFromArchive(newVersion, channel.Value, updateOverride, forced, zipArchive).ConfigureAwait(false)) {
							TickrLogger.LogGenericError(Strings.WarningFailed);
						}
					}
				}
			} catch (Exception e) {
				...
				if (kestrelWasRunning) {
					// We've temporarily disabled TickrKestrel but the update has failed, let's bring it back up
					// We can't even be sure if it's possible to bring it back up in this state, but it's worth trying anyway
					try {
						await TickrKestrel.Start().ConfigureAwait(false);
					} catch (Exception ex) {
						TickrLogger.LogGenericWarningException(ex);
					}
				}
			...
			TickrLogger.LogGenericInfo(Strings.UpdateFinished);
	...
	private static async Task<bool> UpdateFromArchive(Version newVersion, GlobalConfig.EUpdateChannel updateChannel, bool updateOverride, bool forced, ZipArchive zipArchive) {
		...
		if (SharedInfo.HomeDirectory == AppContext.BaseDirectory) {
			// We're running a build that includes our dependencies in Tickr's home
