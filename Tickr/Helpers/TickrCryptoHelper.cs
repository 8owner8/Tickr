...
public static class TickrCryptoHelper {
	...
	private static byte[] EncryptionKey = Encoding.UTF8.GetBytes(nameof(Tickr));
	...
	internal static void SetEncryptionKey(string key) {
		...
		if (!HasDefaultCryptKey) {
			TickrApp.TickrLogger.LogGenericError(Strings.ErrorAborted);
		...
		if (encryptionKey.Length < MinimumRecommendedCryptKeyBytes) {
			TickrApp.TickrLogger.LogGenericWarning(Strings.FormatWarningTooShortCryptKey(MinimumRecommendedCryptKeyBytes));
		}
