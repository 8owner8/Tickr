...
public sealed class WebBrowser : IDisposable {
	...
	private const ushort ExtendedTimeout = 600; // Defines timeout for WebBrowsers dealing with huge data (Tickr update)
	...
	private readonly TickrLogger TickrLogger;
	private readonly HttpClient HttpClient;
	private readonly HttpMessageHandler HttpMessageHandler;

	internal WebBrowser(TickrLogger tickrLogger, IWebProxy? webProxy = null, bool extendedTimeout = false) {
		ArgumentNullException.ThrowIfNull(tickrLogger);

		TickrLogger = tickrLogger;
	...
	public HttpClient GenerateDisposableHttpClient(bool extendedTimeout = false) {
		...
		HttpClient result = new(HttpMessageHandler, false) {
			DefaultRequestHeaders = {
				...
				// If you by any chance came here with a very "clever" idea of hiding your ass by changing default Tickr user-agent then here is a very good advice from me: don't, for your own safety - you've been warned
				UserAgent = {
	...
	public async Task<BinaryResponse?> UrlGetToBinary(Uri request, IReadOnlyCollection<KeyValuePair<string, string>>? headers = null, Uri? referer = null, ERequestOptions requestOptions = ERequestOptions.None, byte maxTries = MaxTries, int rateLimitingDelay = 0, IProgress<byte>? progressReporter = null, CancellationToken cancellationToken = default) {
			...
			await using (response.ConfigureAwait(false)) {
				...
				await using (ms.ConfigureAwait(false)) {
					...
					try {
						...
						TickrLogger.LogGenericDebug(Strings.FormatErrorFailingRequest(Debugging.IsUserDebugging ? request : request.GetLeftPart(UriPartial.Path)));
		...
		TickrLogger.LogGenericWarning(Strings.FormatErrorRequestFailedTooManyTimes(maxTries));
		TickrLogger.LogGenericDebug(Strings.FormatErrorFailingRequest(Debugging.IsUserDebugging ? request : request.GetLeftPart(UriPartial.Path)));
		...
		for (byte i = 0; i < maxTries; i++) {
			...
			await using (response.ConfigureAwait(false)) {
				...
				if (obj is null) {
					...
					TickrLogger.LogGenericWarning(Strings.FormatErrorIsEmpty(nameof(obj)));
	...
	private async Task<HttpResponseMessage?> InternalRequest<T>(Uri request, HttpMethod httpMethod, IReadOnlyCollection<KeyValuePair<string, string>>? headers = null, T? data = null, Uri? referer = null, ERequestOptions requestOptions = ERequestOptions.None, HttpCompletionOption httpCompletionOption = HttpCompletionOption.ResponseContentRead, byte maxRedirections = MaxTries, CancellationToken cancellationToken = default) where T : class {
		...
		while (true) {
			using (HttpRequestMessage requestMessage = new(httpMethod, request)) {
				...
				if (Debugging.IsUserDebugging) {
					TickrLogger.LogGenericDebug($"{httpMethod} {request}");
				}
			...
			if (Debugging.IsUserDebugging) {
				TickrLogger.LogGenericDebug($"{response.StatusCode} <- {httpMethod} {request}");
			}
			...
			if (response.StatusCode.IsRedirectionCode() && (maxRedirections > 0)) {
				...
				if (redirectUri == null) {
					TickrLogger.LogNullError(redirectUri);
				...
				if (redirectUri.IsAbsoluteUri) {
					switch (redirectUri.Scheme) {
						...
						default:
							...
							TickrLogger.LogGenericError(Strings.FormatWarningUnknownValuePleaseReport(nameof(redirectUri.Scheme), redirectUri.Scheme));
		...
		if (!Debugging.IsUserDebugging) {
			TickrLogger.LogGenericDebug($"{response.StatusCode} <- {httpMethod} {request.GetLeftPart(UriPartial.Path)}");
		}

		if (response.StatusCode.IsClientErrorCode()) {
			if (Debugging.IsUserDebugging) {
				TickrLogger.LogGenericDebug(Strings.FormatContent(await response.Content.ReadAsStringAsync(cancellationToken).ConfigureAwait(false)));
			}
