		...
		private init {
			...
			foreach (string creatorIDText in value) {
				if (!ulong.TryParse(creatorIDText, out ulong creatorID) || (creatorID == 0)) {
					TickrApp.TickrLogger.LogGenericError(Strings.FormatErrorIsInvalid(nameof(SAcceptedCreatorIDs)));
