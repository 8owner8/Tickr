...
[Route("Api/Bot")]
public sealed class BotController : TickrController {
	...
	[EndpointSummary("Fetches the complete Steam game library visible to the connected account")]
	[HttpGet("{botName:required}/Games")]
	[ProducesResponseType<GenericResponse<IReadOnlyList<OwnedGame>>>((int) HttpStatusCode.OK)]
	[ProducesResponseType<GenericResponse>((int) HttpStatusCode.BadRequest)]
	[ProducesResponseType<GenericResponse>((int) HttpStatusCode.ServiceUnavailable)]
	public async Task<ActionResult<GenericResponse>> GamesGet(string botName) {
		...
		IReadOnlyList<OwnedGame>? games = await bot.TickrHandler.GetOwnedGamesDetailed(bot.SteamID).ConfigureAwait(false);

		return games != null
			? Ok(new GenericResponse<IReadOnlyList<OwnedGame>>(games))
			: StatusCode((int) HttpStatusCode.ServiceUnavailable, new GenericResponse(false, Strings.WarningFailed));
	}

	[EndpointSummary("Starts hour boosting for the selected Steam games")]
	[HttpPost("{botName:required}/Farming/Start")]
	[ProducesResponseType<GenericResponse>((int) HttpStatusCode.OK)]
	[ProducesResponseType<GenericResponse>((int) HttpStatusCode.BadRequest)]
	public async Task<ActionResult<GenericResponse>> FarmingStartPost(string botName, [FromBody] BotFarmingSelectionRequest request) {
		ArgumentException.ThrowIfNullOrEmpty(botName);
		ArgumentNullException.ThrowIfNull(request);

		Bot? bot = Bot.GetBot(botName);
		if (bot == null) return BadRequest(new GenericResponse(false, Strings.FormatBotNotFound(botName)));
		if (!bot.IsConnectedAndLoggedOn) return StatusCode((int) HttpStatusCode.ServiceUnavailable, new GenericResponse(false, Strings.BotNotConnected));
		if ((request.AppIDs.Count == 0) || request.AppIDs.Contains(0)) return BadRequest(new GenericResponse(false, Strings.FormatErrorIsInvalid(nameof(request.AppIDs))));

		if (request.AppIDs.Count > TickrHandler.MaxGamesPlayedConcurrently) {
			return BadRequest(new GenericResponse(false, $"Steam allows at most {TickrHandler.MaxGamesPlayedConcurrently} games at once."));
		}

		bool success = await bot.StartHourBoosting(request.AppIDs).ConfigureAwait(false);
		return success ? Ok(new GenericResponse(true)) : BadRequest(new GenericResponse(false, Strings.WarningFailed));
	}

	[EndpointSummary("Stops hour boosting for the selected Steam account")]
	[HttpPost("{botName:required}/Farming/Stop")]
	[ProducesResponseType<GenericResponse>((int) HttpStatusCode.OK)]
	[ProducesResponseType<GenericResponse>((int) HttpStatusCode.BadRequest)]
	public async Task<ActionResult<GenericResponse>> FarmingStopPost(string botName) {
		ArgumentException.ThrowIfNullOrEmpty(botName);

		Bot? bot = Bot.GetBot(botName);
		if (bot == null) return BadRequest(new GenericResponse(false, Strings.FormatBotNotFound(botName)));

		bool success = await bot.StopHourBoosting().ConfigureAwait(false);
		return success ? Ok(new GenericResponse(true)) : BadRequest(new GenericResponse(false, Strings.WarningFailed));
	}
	...
	public async Task<ActionResult<GenericResponse>> BotPost([Description(WebUtilities.BotNamesParameterDescription)] string botNames, [FromBody] BotRequest request) {
		...
		if (bots.Any(static botName => !TickrApp.IsValidBotName(botName))) {
	...
	[EndpointSummary("Creates a bot from Steam credentials and protects the password for the current Windows user")]
	[HttpPost("{botName:required}/Credentials")]
	[ProducesResponseType<GenericResponse>((int) HttpStatusCode.OK)]
	[ProducesResponseType<GenericResponse>((int) HttpStatusCode.BadRequest)]
	public async Task<ActionResult<GenericResponse>> CredentialsPost(string botName, [FromBody] BotCredentialsLoginRequest request) {
		ArgumentException.ThrowIfNullOrEmpty(botName);
		ArgumentNullException.ThrowIfNull(request);

		if (!TickrApp.IsValidBotName(botName) || string.IsNullOrWhiteSpace(request.SteamLogin) || string.IsNullOrEmpty(request.SteamPassword)) {
			return BadRequest(new GenericResponse(false, Strings.FormatErrorIsInvalid(nameof(request))));
		}

		string? protectedPassword = TickrCryptoHelper.Encrypt(TickrCryptoHelper.ECryptoMethod.ProtectedDataForCurrentUser, request.SteamPassword);
		if (string.IsNullOrEmpty(protectedPassword)) return BadRequest(new GenericResponse(false, Strings.WarningFailed));

		// Keep an unfinished password login disabled on disk. We start it explicitly below,
		// and only persist Enabled=true after Steam has accepted the credentials and Guard code.
		// This prevents Steam from sending a new Guard email merely because Tickr was opened.
		BotConfig config = new() { Enabled = false };
		config.PasswordFormat = TickrCryptoHelper.ECryptoMethod.ProtectedDataForCurrentUser;
		config.SteamLogin = request.SteamLogin;
		config.SteamPassword = protectedPassword;
		config.Saving = true;

		string? filePath = Bot.GetFilePath(botName, Bot.EFileType.Config);
		if (string.IsNullOrEmpty(filePath)) throw new InvalidOperationException(nameof(filePath));

		bool success = await BotConfig.Write(filePath, config).ConfigureAwait(false);
		if (!success) return BadRequest(new GenericResponse(false, Strings.WarningFailed));

		Bot? bot = null;
		for (byte attempt = 0; attempt < 50; attempt++) {
			Bot? candidate = Bot.GetBot(botName);
			if ((candidate != null) && !candidate.BotConfig.Enabled && string.Equals(candidate.BotConfig.SteamLogin, request.SteamLogin, StringComparison.Ordinal)) {
				bot = candidate;
				break;
			}
		...
		if (bot == null) return StatusCode((int) HttpStatusCode.ServiceUnavailable, new GenericResponse(false, Strings.WarningFailed));
		await bot.Start().ConfigureAwait(false);
		return Ok(new GenericResponse(true));
	}

	[EndpointSummary("Imports .maFile (Steam mobile authenticator) for given bot")]
	[HttpPost("{botName:required}/MaFile")]
	[ProducesResponseType<GenericResponse>((int) HttpStatusCode.OK)]
	[ProducesResponseType<GenericResponse>((int) HttpStatusCode.BadRequest)]
	public async Task<ActionResult<GenericResponse>> MaFilePost(string botName, [FromBody] BotMaFileRequest request) {
		ArgumentException.ThrowIfNullOrEmpty(botName);
		ArgumentNullException.ThrowIfNull(request);
		...
		if (string.IsNullOrWhiteSpace(request.MaFile)) {
			return BadRequest(new GenericResponse(false, Strings.FormatErrorIsEmpty(nameof(request.MaFile))));
		}

		if (!TickrApp.IsValidBotName(botName)) {
			return BadRequest(new GenericResponse(false, Strings.FormatErrorIsInvalid(nameof(botName))));
		}

		// Validate that the provided maFile is a valid Steam mobile authenticator JSON before persisting it
		MobileAuthenticator? authenticator;

		try {
			authenticator = request.MaFile.ToJsonObject<MobileAuthenticator>();
		} catch (Exception e) {
		...
		if (authenticator == null) {
			return BadRequest(new GenericResponse(false, Strings.FormatErrorIsInvalid(nameof(request.MaFile))));
		}

		string filePath = Bot.GetFilePath(botName, Bot.EFileType.MobileAuthenticator);

		if (string.IsNullOrEmpty(filePath)) {
			throw new InvalidOperationException(nameof(filePath));
		}

		await System.IO.File.WriteAllTextAsync(filePath, request.MaFile).ConfigureAwait(false);

		// Restart the bot (if registered) so that the authenticator gets imported right away
		if (Bot.Bots.TryGetValue(botName, out Bot? bot)) {
			await bot.Stop(true).ConfigureAwait(false);
			await bot.Start().ConfigureAwait(false);
		}
	...
	public async Task<ActionResult<GenericResponse>> InputPost([Description(WebUtilities.BotNamesParameterDescription)] string botNames, [FromBody] BotInputRequest request) {
		...
		if ((request.Type == TickrApp.EUserInputType.None) || !Enum.IsDefined(request.Type) || string.IsNullOrEmpty(request.Value)) {
	...
	public async Task<ActionResult<GenericResponse>> InventoryInfoGet([Description(WebUtilities.BotNamesParameterDescription)] string botNames) {
		...
		IList<ImmutableDictionary<uint, InventoryAppData>?> results = await Utilities.InParallel(bots.Select(static bot => bot.TickrWebHandler.GetInventoryContextData())).ConfigureAwait(false);
	...
	[EndpointDescription("Response contains a map that maps each provided cd-key to its redeem result. Redeem result can be a null value, this means that Tickr didn't even attempt to send a request (e.g. because of bot not being connected to Steam network)")]
	...
	public async Task<ActionResult<GenericResponse>> RenamePost([Description(WebUtilities.BotNamesParameterDescription)] string botName, [FromBody] BotRenameRequest request) {
		...
		if (string.IsNullOrEmpty(request.NewName) || !TickrApp.IsValidBotName(request.NewName) || Bot.Bots.ContainsKey(request.NewName)) {
