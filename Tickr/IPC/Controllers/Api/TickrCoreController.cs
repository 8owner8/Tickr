...
[Route("Api/Tickr")]
public sealed class TickrCoreController : TickrController {
	internal static Version? PendingVersionUpdate { get; set; }

	private readonly IHostApplicationLifetime ApplicationLifetime;

	public TickrCoreController(IHostApplicationLifetime applicationLifetime) {
		ArgumentNullException.ThrowIfNull(applicationLifetime);

		ApplicationLifetime = applicationLifetime;
	}

	[EndpointSummary("Encrypts data with Tickr encryption mechanisms using provided details.")]
	[HttpPost("Encrypt")]
	[ProducesResponseType<GenericResponse<string>>((int) HttpStatusCode.OK)]
	[ProducesResponseType<GenericResponse>((int) HttpStatusCode.BadRequest)]
	public ActionResult<GenericResponse> TickrEncryptPost([FromBody] TickrEncryptRequest request) {
	...
	[EndpointSummary("Fetches common info related to Tickr as a whole")]
	[HttpGet]
	[ProducesResponseType<GenericResponse<TickrResponse>>((int) HttpStatusCode.OK)]
	public ActionResult<GenericResponse<TickrResponse>> TickrGet() {
		...
		TickrResponse result = new(BuildInfo.Variant, BuildInfo.CanUpdate, TickrApp.GlobalConfig, memoryUsage, OS.ProcessStartTime, SharedInfo.Version);

		return Ok(new GenericResponse<TickrResponse>(result));
	}

	[EndpointSummary("Hashes data with Tickr hashing mechanisms using provided details")]
	[HttpPost("Hash")]
	[ProducesResponseType<GenericResponse<string>>((int) HttpStatusCode.OK)]
	[ProducesResponseType<GenericResponse>((int) HttpStatusCode.BadRequest)]
	public ActionResult<GenericResponse> TickrHashPost([FromBody] TickrHashRequest request) {
		ArgumentNullException.ThrowIfNull(request);
	...
	[EndpointSummary("Updates Tickr's global config")]
	[HttpPost]
	[ProducesResponseType<GenericResponse>((int) HttpStatusCode.OK)]
	[ProducesResponseType<GenericResponse>((int) HttpStatusCode.BadRequest)]
	public async Task<ActionResult<GenericResponse>> TickrPost([FromBody] TickrRequest request) {
		ArgumentNullException.ThrowIfNull(request);
		...
		if (!request.GlobalConfig.IsIPCPasswordSet && TickrApp.GlobalConfig.IsIPCPasswordSet) {
			request.GlobalConfig.IPCPassword = TickrApp.GlobalConfig.IPCPassword;
		}

		if (!request.GlobalConfig.IsLicenseIDSet && TickrApp.GlobalConfig.IsLicenseIDSet) {
			request.GlobalConfig.LicenseID = TickrApp.GlobalConfig.LicenseID;
		}

		if (!request.GlobalConfig.IsWebProxyPasswordSet && TickrApp.GlobalConfig.IsWebProxyPasswordSet) {
			request.GlobalConfig.WebProxyPassword = TickrApp.GlobalConfig.WebProxyPassword;
		}

		if (TickrApp.GlobalConfig.AdditionalProperties is { Count: > 0 }) {
			request.GlobalConfig.AdditionalProperties ??= new Dictionary<string, JsonElement>(TickrApp.GlobalConfig.AdditionalProperties.Count, TickrApp.GlobalConfig.AdditionalProperties.Comparer);

			foreach ((string key, JsonElement value) in TickrApp.GlobalConfig.AdditionalProperties.Where(property => !request.GlobalConfig.AdditionalProperties.ContainsKey(property.Key))) {
		...
		string filePath = TickrApp.GetFilePath(TickrApp.EFileType.Config);

		if (string.IsNullOrEmpty(filePath)) {
			TickrApp.TickrLogger.LogNullError(filePath);
	...
	[EndpointSummary("Makes Tickr shutdown itself")]
	[HttpPost("Exit")]
	[ProducesResponseType<GenericResponse>((int) HttpStatusCode.OK)]
	...
	[EndpointSummary("Makes Tickr restart itself")]
	[HttpPost("Restart")]
	[ProducesResponseType<GenericResponse>((int) HttpStatusCode.OK)]
	...
	[EndpointSummary("Makes Tickr update itself")]
	[HttpPost("Update")]
	[ProducesResponseType<GenericResponse<string>>((int) HttpStatusCode.OK)]
