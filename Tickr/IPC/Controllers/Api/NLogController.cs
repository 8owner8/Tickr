...
[Route("Api/NLog")]
public sealed class NLogController : TickrController {
	...
	[EndpointSummary("Fetches Tickr log file")]
	[HttpGet("File")]
	...
	[EndpointSummary("Fetches Tickr log in realtime")]
	...
	public async Task<ActionResult> Get() {
		...
		try {
			...
			try {
				...
				if (TickrKestrel.HistoryTarget != null) {
					...
					await Task.WhenAll(TickrKestrel.HistoryTarget.ArchivedMessages.Select(archivedMessage => PostLoggedMessageUpdate(webSocket, archivedMessage, sendSemaphore, cancellationTokenSource.Token))).ConfigureAwait(false);
