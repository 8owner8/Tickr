...
namespace Tickr.IPC.Integration;
...
internal sealed class ApiAuthenticationMiddleware {
	...
	private async Task<(HttpStatusCode StatusCode, bool Permanent)> GetAuthenticationStatus(HttpContext context) {
		...
		string? ipcPassword = TickrApp.GlobalConfig != null ? TickrApp.GlobalConfig.IPCPassword : GlobalConfig.DefaultIPCPassword;
		...
		TickrCryptoHelper.EHashingMethod ipcPasswordFormat = TickrApp.GlobalConfig != null ? TickrApp.GlobalConfig.IPCPasswordFormat : GlobalConfig.DefaultIPCPasswordFormat;

		bool authorized = TickrCryptoHelper.VerifyHash(ipcPasswordFormat, inputPassword, ipcPassword);
