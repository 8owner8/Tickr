...
internal static class TickrKestrel {
	...
	private static void ConfigureApp([SuppressMessage("ReSharper", "SuggestBaseTypeForParameter")] ConfigurationManager configuration, IWebHostEnvironment environment, WebApplication app) {
		...
		if (TickrApp.GlobalConfig?.OptimizationMode != GlobalConfig.EOptimizationMode.MinMemoryUsage) {
		...
		foreach (IWebInterface plugin in PluginsCore.ActivePlugins.OfType<IWebInterface>()) {
			...
			if (string.IsNullOrEmpty(physicalPath)) {
				...
				TickrApp.TickrLogger.LogGenericError(Strings.FormatErrorObjectIsNull($"{nameof(physicalPath)} ({plugin.Name})"));
			...
			if (string.IsNullOrEmpty(webPath)) {
				// Invalid path provided
				TickrApp.TickrLogger.LogGenericError(Strings.FormatErrorObjectIsNull($"{nameof(webPath)} ({plugin.Name})"));
			...
			if (!Directory.Exists(physicalPath)) {
				...
				TickrApp.TickrLogger.LogGenericWarning(Strings.FormatErrorIsInvalid($"{nameof(physicalPath)} ({plugin.Name})"));
		...
		// Add support for CORS policy in order to allow userscripts and other third-party integrations (including GUI loaded over file://) to communicate with Tickr API
		// This must be applied before API authentication middleware, so that preflight OPTIONS requests are handled by CORS and not rejected with 401
		app.UseCors();
		...
		app.MapScalarApiReference(
			"/swagger", static options => {
				...
				options.OpenApiRoutePattern = $"/swagger/{SharedInfo.Tickr}/swagger.json";
	...
	private static void ConfigureServices([SuppressMessage("ReSharper", "SuggestBaseTypeForParameter")] ConfigurationManager configuration, IServiceCollection services) {
		...
		if (knownNetworksTexts?.Count > 0) {
			...
			foreach (string knownNetworkText in knownNetworksTexts) {
				...
				if ((addressParts.Length != 2) || !IPAddress.TryParse(addressParts[0], out IPAddress? ipAddress) || !byte.TryParse(addressParts[1], out byte prefixLength)) {
					TickrApp.TickrLogger.LogGenericError(Strings.FormatErrorIsInvalid(nameof(knownNetworkText)));
					TickrApp.TickrLogger.LogGenericDebug($"{nameof(knownNetworkText)}: {knownNetworkText}");
		...
		// Add support for CORS policy in order to allow userscripts and other third-party integrations (including GUI loaded over file://) to communicate with Tickr API
		services.AddCors(static options => options.AddDefaultPolicy(static policyBuilder => policyBuilder.AllowAnyOrigin().AllowAnyHeader().AllowAnyMethod()));
		...
		services.AddOpenApi(
			SharedInfo.Tickr, static options => {
	...
	private static async Task<WebApplication> CreateWebApplication() {
			...
			if (Debugging.IsDebugConfigured) {
				try {
					...
					if (!string.IsNullOrEmpty(json)) {
						...
						TickrApp.TickrLogger.LogGenericDebug($"{SharedInfo.IPCConfigFile}: {jsonNode?.ToJsonText(true) ?? "null"}");
					}
				} catch (Exception e) {
					TickrApp.TickrLogger.LogGenericException(e);
				}
		...
		builder.WebHost.ConfigureKestrel(options => {
				...
				if (customConfigExists) {
					...
					// Use backend defaults for Kestrel
	...
	private static void OnPrepareResponse(StaticFileResponseContext context) {
		...
		switch (extension.ToUpperInvariant()) {
			...
			default:
				...
				// This is used to handle Tickr and user updates to WWW root, we don't want the client to ever use outdated scripts
	...
	private static async Task StartInternally() {
		...
		TickrApp.TickrLogger.LogGenericInfo(Strings.IPCStarting);
		...
		TickrApp.TickrLogger.LogGenericInfo(Strings.IPCReady);
	}
