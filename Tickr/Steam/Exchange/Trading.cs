...
public sealed class Trading : IDisposable {
	...
	public async Task<bool> AcknowledgeTradeRestrictions() {
		...
		return Bot.BotDatabase.TradeRestrictionsAcknowledged = await Bot.TickrWebHandler.AcknowledgeTradeRestrictions().ConfigureAwait(false);
	}
	...
	private async Task<bool> ParseActiveTrades() {
		...
		for (byte i = 0; i < WebBrowser.MaxTries; i++) {
			HashSet<TradeOffer>? tradeOffers = await Bot.TickrWebHandler.GetTradeOffers(true, true, false, true).ConfigureAwait(false);
			...
			// If any trade asked to be retried, we do have Tickr 2FA and actually managed to confirm something (else), then now is the time for retry
	...
	private async Task<ParseTradeResult> ParseTrade(TradeOffer tradeOffer, ISet<(uint RealAppID, EAssetType Type, EAssetRarity Rarity)> handledSets) {
		...
		switch (result) {
			case ParseTradeResult.EResult.Accepted:
				Bot.TickrLogger.LogGenericInfo(Strings.FormatAcceptingTrade(tradeOffer.TradeOfferID));

				(bool success, bool requiresMobileConfirmation) = await Bot.TickrWebHandler.AcceptTradeOffer(tradeOffer.TradeOfferID).ConfigureAwait(false);
				...
				if (tradeOffer.ItemsToReceive.Sum(static item => item.Amount) > tradeOffer.ItemsToGive.Sum(static item => item.Amount)) {
					Bot.TickrLogger.LogGenericTrace(Strings.FormatBotAcceptedDonationTrade(tradeOffer.TradeOfferID));
				}
			...
			case ParseTradeResult.EResult.Rejected when Bot.BotConfig.BotBehaviour.HasFlag(BotConfig.EBotBehaviour.RejectInvalidTrades):
				Bot.TickrLogger.LogGenericInfo(Strings.FormatRejectingTrade(tradeOffer.TradeOfferID));

				if (!await Bot.TickrWebHandler.DeclineTradeOffer(tradeOffer.TradeOfferID).ConfigureAwait(false)) {
					result = ParseTradeResult.EResult.TryAgain;
			...
			case ParseTradeResult.EResult.Rejected:
				...
				Bot.TickrLogger.LogGenericInfo(Strings.FormatIgnoringTrade(tradeOffer.TradeOfferID));
			...
			default:
				Bot.TickrLogger.LogGenericError(Strings.FormatWarningUnknownValuePleaseReport(nameof(result), result));
	...
	private async Task<ParseTradeResult.EResult> ShouldAcceptTrade(TradeOffer tradeOffer, ISet<(uint RealAppID, EAssetType Type, EAssetRarity Rarity)> handledSets) {
		...
		if (tradeOffer.OtherSteamID64 != 0) {
			...
			if (Bot.IsBlacklistedFromTrades(tradeOffer.OtherSteamID64)) {
				Bot.TickrLogger.LogGenericDebug(Strings.FormatBotTradeOfferResult(tradeOffer.TradeOfferID, ParseTradeResult.EResult.Blacklisted, $"{nameof(tradeOffer.OtherSteamID64)} {tradeOffer.OtherSteamID64}"));
			...
			if (Bot.GetAccess(tradeOffer.OtherSteamID64) >= EAccess.Master) {
				Bot.TickrLogger.LogGenericDebug(Strings.FormatBotTradeOfferResult(tradeOffer.TradeOfferID, ParseTradeResult.EResult.Accepted, $"{nameof(tradeOffer.OtherSteamID64)} {tradeOffer.OtherSteamID64}: {BotConfig.EAccess.Master}"));

				return ParseTradeResult.EResult.Accepted;
			}

			// Deny trades from bad steamIDs if user wishes to do so
			if (TickrApp.GlobalConfig?.FilterBadBots ?? GlobalConfig.DefaultFilterBadBots) {
				...
				bool? isBadBot = await TickrNet.IsBadBot(tradeOffer.OtherSteamID64, tickrNetCancellation.Token).ConfigureAwait(false);
		...
		switch (tradeOffer.ItemsToGive.Count) {
			case 0 when tradeOffer.ItemsToReceive.Count == 0:
				...
				Bot.TickrLogger.LogGenericDebug(Strings.FormatBotTradeOfferResult(tradeOffer.TradeOfferID, ParseTradeResult.EResult.TryAgain, $"{nameof(tradeOffer.ItemsToReceive.Count)} = 0"));
			...
			case 0:
				...
				switch (acceptDonations) {
					case true when acceptBotTrades:
						...
						Bot.TickrLogger.LogGenericDebug(Strings.FormatBotTradeOfferResult(tradeOffer.TradeOfferID, ParseTradeResult.EResult.Accepted, $"{nameof(acceptDonations)} = {true} && {nameof(acceptBotTrades)} = {true}"));

						return ParseTradeResult.EResult.Accepted;

					case false when !acceptBotTrades:
						// If we don't accept donations, neither bot trades, deny it right away
						Bot.TickrLogger.LogGenericDebug(Strings.FormatBotTradeOfferResult(tradeOffer.TradeOfferID, ParseTradeResult.EResult.Rejected, $"{nameof(acceptDonations)} = {false} && {nameof(acceptBotTrades)} = {false}"));
				...
				Bot.TickrLogger.LogGenericDebug(Strings.FormatBotTradeOfferResult(tradeOffer.TradeOfferID, result, $"{nameof(acceptDonations)} = {acceptDonations} && {nameof(acceptBotTrades)} = {acceptBotTrades} && {nameof(isBotTrade)} = {isBotTrade}"));
		...
		if (!Bot.BotConfig.TradingPreferences.HasFlag(BotConfig.ETradingPreferences.SteamTradeMatcher)) {
			Bot.TickrLogger.LogGenericDebug(Strings.FormatBotTradeOfferResult(tradeOffer.TradeOfferID, ParseTradeResult.EResult.Rejected, $"{nameof(BotConfig.ETradingPreferences.SteamTradeMatcher)} = {false}"));
		...
		if (tradeOffer.ItemsToGive.Count > tradeOffer.ItemsToReceive.Count) {
			Bot.TickrLogger.LogGenericDebug(Strings.FormatBotTradeOfferResult(tradeOffer.TradeOfferID, ParseTradeResult.EResult.Rejected, $"{nameof(tradeOffer.ItemsToGive.Count)}: {tradeOffer.ItemsToGive.Count} > {tradeOffer.ItemsToReceive.Count}"));
		...
		if (Bot.BotConfig.MatchableTypes.IsEmpty || !tradeOffer.IsValidSteamItemsRequest(Bot.BotConfig.MatchableTypes) || !IsFairExchange(tradeOffer.ItemsToGive, tradeOffer.ItemsToReceive)) {
			Bot.TickrLogger.LogGenericDebug(Strings.FormatBotTradeOfferResult(tradeOffer.TradeOfferID, ParseTradeResult.EResult.Rejected, $"{nameof(tradeOffer.IsValidSteamItemsRequest)} || {nameof(IsFairExchange)}"));
		...
		if (hasBlacklistedCards || (maxTradeHoldDuration < MaxTradeHoldDuration)) {
			...
			switch (holdDuration) {
				case null:
					...
					Bot.TickrLogger.LogGenericDebug(Strings.FormatBotTradeOfferResult(tradeOffer.TradeOfferID, ParseTradeResult.EResult.TryAgain, nameof(holdDuration)));
				...
				case > 0 when hasBlacklistedCards || (holdDuration.Value > maxTradeHoldDuration):
					...
					Bot.TickrLogger.LogGenericDebug(Strings.FormatBotTradeOfferResult(tradeOffer.TradeOfferID, ParseTradeResult.EResult.Rejected, $"{nameof(holdDuration)} > 0: {holdDuration.Value}"));
		...
		if (Bot.BotConfig.TradingPreferences.HasFlag(BotConfig.ETradingPreferences.MatchEverything)) {
			Bot.TickrLogger.LogGenericDebug(Strings.FormatBotTradeOfferResult(tradeOffer.TradeOfferID, ParseTradeResult.EResult.Accepted, BotConfig.ETradingPreferences.MatchEverything));
		...
		try {
			inventory = await Bot.TickrHandler.GetMyInventoryAsync().Where(item => !item.IsSteamPointsShopItem && wantedSets.Contains((item.RealAppID, item.Type, item.Rarity))).ToHashSetAsync().ConfigureAwait(false);
			...
			Bot.TickrLogger.LogGenericDebug(Strings.FormatBotTradeOfferResult(tradeOffer.TradeOfferID, ParseTradeResult.EResult.TryAgain, nameof(inventory)));
		...
		if (inventory.Count == 0) {
			// If we can't check our inventory when not using MatchEverything, this is a temporary failure, try again later
			Bot.TickrLogger.LogGenericWarning(Strings.FormatErrorIsEmpty(nameof(inventory)));
			Bot.TickrLogger.LogGenericDebug(Strings.FormatBotTradeOfferResult(tradeOffer.TradeOfferID, ParseTradeResult.EResult.TryAgain, nameof(inventory)));
		...
		if (accept) {
			...
			lock (handledSets) {
				if (handledSets.Overlaps(wantedSets)) {
					Bot.TickrLogger.LogGenericDebug(Strings.FormatBotTradeOfferResult(tradeOffer.TradeOfferID, ParseTradeResult.EResult.RetryAfterOthers, nameof(handledSets)));
		...
		Bot.TickrLogger.LogGenericDebug(Strings.FormatBotTradeOfferResult(tradeOffer.TradeOfferID, acceptResult, nameof(IsTradeNeutralOrBetter)));
