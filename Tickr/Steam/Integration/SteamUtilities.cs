...
public static class SteamUtilities {
	...
	internal static EResult? InterpretError(string errorText) {
		...
		if (match.Success && match.Groups.TryGetValue("EResult", out Group? groupResult)) {
			...
			if (startIndex < 0) {
				TickrApp.TickrLogger.LogGenericError(Strings.FormatWarningUnknownValuePleaseReport(nameof(errorText), errorText));
	...
	internal static bool TryParseGameIdentifier(string input, EGameIdentifier defaultType, [NotNullWhen(true)] out EGameIdentifier? type, [NotNullWhen(true)] out string? value) {
		...
		if (Uri.TryCreate(input, UriKind.Absolute, out Uri? uri)) {
			switch (uri.Scheme) {
				case "http" or "https" when uri.Host.Equals(TickrWebHandler.SteamStoreURL.Host, StringComparison.OrdinalIgnoreCase):
		...
		// Handle common Tickr-specific format of <format>/<input>
