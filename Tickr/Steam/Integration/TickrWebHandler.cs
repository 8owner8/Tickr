...
public sealed class TickrWebHandler : IDisposable {
	// Steam network (TickrHandler) may work unstable with more items than this (throwing upon description details), while Steam web (TickrWebHandler) forcefully limits to this value maximum
	...
	[PublicAPI]
	public static ushort WebLimiterDelay => TickrApp.GlobalConfig?.WebLimiterDelay ?? GlobalConfig.DefaultWebLimiterDelay;
	...
	internal TickrWebHandler(Bot bot) {
		ArgumentNullException.ThrowIfNull(bot);

		Bot = bot;
		WebBrowser = new WebBrowser(bot.TickrLogger, bot.BotConfig.WebProxy ?? TickrApp.GlobalConfig?.WebProxy);
	}
	...
	public async Task<(bool Success, bool RequiresMobileConfirmation)> AcceptTradeOffer(ulong tradeID) {
		...
		for (byte i = 0; (i < WebBrowser.MaxTries) && (response == null); i++) {
			...
			if (response.StatusCode.IsServerErrorCode()) {
				...
				Bot.TickrLogger.LogGenericWarning(Strings.FormatWarningFailedWithError(response.Content.ErrorText));
	...
	public async Task<ImmutableHashSet<BoosterCreatorEntry>?> GetBoosterCreatorEntries() {
		...
		if (scriptNodes.Length == 0) {
			Bot.TickrLogger.LogNullError(scriptNodes);
		...
		foreach (IElement scriptNode in scriptNodes) {
			...
			if (endIndex <= startIndex) {
				Bot.TickrLogger.LogNullError(endIndex);
		...
		if (result == null) {
			Bot.TickrLogger.LogNullError(result);
	...
	public async Task<HashSet<uint>?> GetBoosterEligibility() {
		...
		foreach (string? hrefText in linkNodes.Select(static linkNode => linkNode.GetAttribute("href"))) {
			if (string.IsNullOrEmpty(hrefText)) {
				Bot.TickrLogger.LogNullError(hrefText);
			...
			if ((index <= 0) || (hrefText.Length <= index + 2)) {
				Bot.TickrLogger.LogNullError(index);
	...
	/// <remarks>
	///     If targetting inventory of this <see cref="Bot" /> instance, consider using much more efficient <see cref="TickrHandler.GetMyInventoryAsync" /> instead.
	...
	public async IAsyncEnumerable<Asset> GetInventoryAsync(ulong steamID = 0, uint appID = Asset.SteamAppID, ulong contextID = Asset.SteamCommunityContextID, ushort itemsCountPerRequest = MaxItemsInSingleInventoryRequest, string? language = null) {
		...
		if (TickrApp.InventorySemaphore == null) {
			throw new InvalidOperationException(nameof(TickrApp.InventorySemaphore));
		}
		...
		int rateLimitingDelay = (TickrApp.GlobalConfig?.InventoryLimiterDelay ?? GlobalConfig.DefaultInventoryLimiterDelay) * 1000;
		...
		while (true) {
			...
			await TickrApp.InventorySemaphore.WaitAsync().ConfigureAwait(false);
			...
			try {
				for (byte i = 0; (i < WebBrowser.MaxTries) && (response?.Content is not { Result: EResult.OK } || !response.StatusCode.IsSuccessCode()); i++) {
					...
					if (response.StatusCode.IsServerErrorCode()) {
						...
						Bot.TickrLogger.LogGenericDebug(Strings.FormatWarningFailedWithError(response.Content.ErrorText));
						...
						switch (response.Content.ErrorCode) {
							...
							default:
								...
								Bot.TickrLogger.LogGenericError(Strings.FormatWarningUnknownValuePleaseReport(nameof(response.Content.ErrorText), response.Content.ErrorText));
				...
				if (rateLimitingDelay == 0) {
					TickrApp.InventorySemaphore.Release();
				} else {
	...
	public async Task<ImmutableDictionary<uint, InventoryAppData>?> GetInventoryContextData() {
		...
		if (string.IsNullOrEmpty(text)) {
			Bot.TickrLogger.LogNullError(text);
		...
		if (startIndex < 0) {
			Bot.TickrLogger.LogNullError(startIndex);
	...
	public async Task<HashSet<TradeOffer>?> GetTradeOffers(bool? activeOffers = null, bool? receivedOffers = null, bool? sentOffers = null, bool? withDescriptions = null) {
		...
		for (byte i = 0; (i < WebBrowser.MaxTries) && (tradeOffersResponse == null); i++) {
			...
			if (response.StatusCode == HttpStatusCode.TooManyRequests) {
				Bot.TickrLogger.LogGenericDebug(Strings.FormatWarningWorkaroundTriggered($"{nameof(GetTradeOffers)}/{response.StatusCode}"));
		...
		if (tradeOffersResponse == null) {
			Bot.TickrLogger.LogGenericWarning(Strings.FormatErrorRequestFailedTooManyTimes(WebBrowser.MaxTries));
		...
		foreach (InventoryDescription description in tradeOffersResponse.Descriptions) {
			if (description.AppID == 0) {
				Bot.TickrLogger.LogNullError(nameof(description.AppID));

				continue;
			}

			if (description.ClassID == 0) {
				Bot.TickrLogger.LogNullError(nameof(description.ClassID));
	...
	public async Task<(bool Success, HashSet<ulong>? TradeOfferIDs, HashSet<ulong>? MobileTradeOfferIDs)> SendTradeOffer(ulong steamID, IReadOnlyCollection<Asset>? itemsToGive = null, IReadOnlyCollection<Asset>? itemsToReceive = null, string? token = null, string? customMessage = null, bool forcedSingleOffer = false, ushort itemsPerTrade = Trading.MaxItemsPerTrade) {
		...
		foreach (TradeOfferSendRequest trade in trades) {
			...
			if (response.Content.TradeOfferID == 0) {
				Bot.TickrLogger.LogNullError(response.Content.TradeOfferID);
	...
	public async Task<HtmlDocumentResponse?> UrlGetToHtmlDocumentWithSession(Uri request, IReadOnlyCollection<KeyValuePair<string, string>>? headers = null, Uri? referer = null, WebBrowser.ERequestOptions requestOptions = WebBrowser.ERequestOptions.None, bool checkSessionPreemptively = true, byte maxTries = WebBrowser.MaxTries, int rateLimitingDelay = 0, bool allowSessionRefresh = true, CancellationToken cancellationToken = default) {
		...
		if (checkSessionPreemptively) {
			...
			if (sessionExpired.GetValueOrDefault(true)) {
				...
				Bot.TickrLogger.LogGenericWarning(Strings.WarningFailed);
				Bot.TickrLogger.LogGenericDebug(Strings.FormatErrorFailingRequest(Debugging.IsUserDebugging ? request : request.GetLeftPart(UriPartial.Path)));
		...
		if (!requestOptions.HasFlag(WebBrowser.ERequestOptions.ReturnRedirections) && await IsProfileUri(response.FinalUri).ConfigureAwait(false) && !await IsProfileUri(request).ConfigureAwait(false)) {
			Bot.TickrLogger.LogGenericDebug(Strings.FormatWarningWorkaroundTriggered(nameof(IsProfileUri)));
	...
	public async Task<HtmlDocumentResponse?> UrlPostToHtmlDocumentWithSession(Uri request, IReadOnlyCollection<KeyValuePair<string, string>>? headers = null, IDictionary<string, string>? data = null, Uri? referer = null, WebBrowser.ERequestOptions requestOptions = WebBrowser.ERequestOptions.None, ESession session = ESession.Lowercase, bool checkSessionPreemptively = true, byte maxTries = WebBrowser.MaxTries, int rateLimitingDelay = 0, bool allowSessionRefresh = true, CancellationToken cancellationToken = default) {
		...
		if (session != ESession.None) {
			...
			if (string.IsNullOrEmpty(sessionID)) {
				Bot.TickrLogger.LogNullError(sessionID);
	...
	public static async Task<T> WebLimitRequest<T>(Uri service, Func<Task<T>> function, CancellationToken cancellationToken = default) {
		...
		if (TickrApp.RateLimitingSemaphore == null) {
			throw new InvalidOperationException(nameof(TickrApp.RateLimitingSemaphore));
		}

		if (TickrApp.WebLimitingSemaphores == null) {
			throw new InvalidOperationException(nameof(TickrApp.WebLimitingSemaphores));
		}
		...
		if (!TickrApp.WebLimitingSemaphores.TryGetValue(service, out (ICrossProcessSemaphore RateLimitingSemaphore, SemaphoreSlim OpenConnectionsSemaphore) limiters)) {
			TickrApp.TickrLogger.LogGenericWarning(Strings.FormatWarningUnknownValuePleaseReport(nameof(service), service));

			limiters.RateLimitingSemaphore = TickrApp.RateLimitingSemaphore;
			limiters.OpenConnectionsSemaphore = TickrApp.OpenConnectionsSemaphore;
		}
	...
	internal async Task<(EResult Result, EPurchaseResultDetail PurchaseResult)> AddFreeLicense(uint subID) {
		...
		switch (response.StatusCode) {
			...
			case HttpStatusCode.OK:
				...
				try {
					...
					switch (errorMessage) {
						...
						default:
							Bot.TickrLogger.LogGenericError(Strings.FormatWarningUnknownValuePleaseReport(nameof(errorMessage), errorMessage));
			...
			default:
				...
				Bot.TickrLogger.LogGenericError(Strings.FormatWarningUnknownValuePleaseReport(nameof(response.StatusCode), response.StatusCode));
		...
		while (true) {
			...
			for (byte i = 0; (i < WebBrowser.MaxTries) && (response == null); i++) {
				...
				if (Debugging.IsUserDebugging) {
					Bot.TickrLogger.LogGenericDebug($"{method} {Bot.SteamConfiguration.WebAPIBaseAddress}{SteamStoreService}/{endpoint}");
				}
			...
			if (apps.Count == 0) {
				Bot.TickrLogger.LogGenericWarning(Strings.FormatErrorIsEmpty(nameof(apps)));
	...
	internal async Task<byte> GetCardCountForGame(uint appID) {
		...
		if (TickrApp.GlobalDatabase?.CardCountsPerGame.TryGetValue(appID, out byte result) == true) {
			return result;
		}
		...
		if (htmlDocument == null) {
			Bot.TickrLogger.LogNullError(htmlDocument);
		...
		if (htmlNodes.Length == 0) {
			Bot.TickrLogger.LogNullError(htmlNodes);
		...
		TickrApp.GlobalDatabase?.CardCountsPerGame.TryAdd(appID, result);
	...
	internal async Task<byte?> GetCombinedTradeHoldDurationAgainstUser(ulong steamID, string? tradeToken = null) {
		...
		for (byte i = 0; (i < WebBrowser.MaxTries) && (response == null); i++) {
			...
			if (Debugging.IsUserDebugging) {
				Bot.TickrLogger.LogGenericDebug($"{method} {Bot.SteamConfiguration.WebAPIBaseAddress}{EconService}/{endpoint}");
			}
		...
		if (resultInSeconds == uint.MaxValue) {
			Bot.TickrLogger.LogNullError(resultInSeconds);
	...
	internal async Task<HashSet<ulong>?> GetDigitalGiftCards() {
		...
		foreach (string? giftCardIDText in htmlNodes.Select(static htmlNode => htmlNode.GetAttribute("id"))) {
			if (string.IsNullOrEmpty(giftCardIDText)) {
				Bot.TickrLogger.LogNullError(giftCardIDText);

				return null;
			}

			if (giftCardIDText.Length <= 13) {
				Bot.TickrLogger.LogGenericError(Strings.FormatErrorIsInvalid(nameof(giftCardIDText)));

				return null;
			}

			if (!ulong.TryParse(giftCardIDText[13..], out ulong giftCardID) || (giftCardID == 0)) {
				Bot.TickrLogger.LogGenericError(Strings.FormatErrorParsingObject(nameof(giftCardID)));
	...
	internal async Task<HashSet<ulong>?> GetFamilySharingSteamIDs() {
		...
		foreach (string? miniProfile in htmlNodes.Select(static htmlNode => htmlNode.GetAttribute("data-miniprofile"))) {
			if (string.IsNullOrEmpty(miniProfile)) {
				Bot.TickrLogger.LogNullError(miniProfile);

				return null;
			}

			if (!uint.TryParse(miniProfile, out uint steamID3) || (steamID3 == 0)) {
				Bot.TickrLogger.LogNullError(steamID3);
	...
	internal async Task MarkInventory() {
		...
		lock (TickrApp.InventorySemaphore) {
	...
	private async Task<bool> RefreshSession() {
		...
		try {
			...
			Bot.TickrLogger.LogGenericInfo(Strings.RefreshingOurSession);
	...
	private async Task<bool> UnlockParentalAccount(string parentalCode) {
		...
		Bot.TickrLogger.LogGenericInfo(Strings.UnlockingParentalAccount);
