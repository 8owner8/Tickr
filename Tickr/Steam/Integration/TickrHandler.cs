...
using Tickr.Steam.Integration.CMsgs;
using Tickr.Web;
...
public sealed class TickrHandler : ClientMsgHandler, IDisposable {
	...
	internal TickrHandler(TickrLogger tickrLogger, SteamUnifiedMessages steamUnifiedMessages) {
	...
	[PublicAPI]
	public async IAsyncEnumerable<Asset> GetMyInventoryAsync(uint appID = Asset.SteamAppID, ulong contextID = Asset.SteamCommunityContextID, bool tradableOnly = false, bool marketableOnly = false, ushort itemsCountPerRequest = TickrWebHandler.MaxItemsInSingleInventoryRequest, string? language = null) {
				...
				for (byte i = 0; (i < WebBrowser.MaxTries) && (response?.Result != EResult.OK) && Client.IsConnected && (Client.SteamID != null); i++) {
					...
					switch (response.Result) {
						...
						default:
							...
							TickrLogger.LogGenericError(Strings.FormatWarningUnknownValuePleaseReport(nameof(response.Result), response.Result));
	...
	public async Task<Dictionary<uint, string>?> GetOwnedGames(ulong steamID) {
		IReadOnlyList<OwnedGame>? games = await GetOwnedGamesDetailed(steamID).ConfigureAwait(false);

		return games?.ToDictionary(static game => game.AppID, static game => game.Name);
	}

	[PublicAPI]
	public async Task<IReadOnlyList<OwnedGame>?> GetOwnedGamesDetailed(ulong steamID) {
		if ((steamID == 0) || !new SteamID(steamID).IsIndividualAccount) {
			throw new ArgumentOutOfRangeException(nameof(steamID));
		}
		...
		return response.Body.games
			.Where(static game => game.appid > 0)
			.Select(static game => new OwnedGame((uint) game.appid, game.name, game.playtime_forever, game.playtime_2weeks, game.rtime_last_played, game.img_icon_url, game.has_community_visible_stats))
			.OrderBy(static game => game.Name, StringComparer.CurrentCultureIgnoreCase)
