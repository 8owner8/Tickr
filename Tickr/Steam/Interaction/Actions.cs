...
namespace Tickr.Steam.Interaction;
...
public sealed class Actions : IAsyncDisposable, IDisposable {
	...
	public async Task<(EResult Result, EPurchaseResultDetail PurchaseResultDetail)> AddFreeLicensePackage(uint subID) {
		...
		return await Bot.TickrWebHandler.AddFreeLicense(subID).ConfigureAwait(false);
	}

	[PublicAPI]
	public static string? Encrypt(TickrCryptoHelper.ECryptoMethod cryptoMethod, string stringToEncrypt) {
		if (!Enum.IsDefined(cryptoMethod)) {
			throw new InvalidEnumArgumentException(nameof(cryptoMethod), (int) cryptoMethod, typeof(TickrCryptoHelper.ECryptoMethod));
		}

		ArgumentException.ThrowIfNullOrEmpty(stringToEncrypt);

		return TickrCryptoHelper.Encrypt(cryptoMethod, stringToEncrypt);
	}
	...
	public async Task<(bool Success, string? Token, string Message)> GenerateTwoFactorAuthenticationToken() {
		if (Bot.BotDatabase.MobileAuthenticator == null) {
			return (false, null, Strings.BotNoTickrAuthenticator);
		}
	...
	public ulong GetFirstSteamMasterID() {
		...
		ulong steamOwnerID = TickrApp.GlobalConfig?.SteamOwnerID ?? GlobalConfig.DefaultSteamOwnerID;

		return (steamOwnerID > 0) && new SteamID(steamOwnerID).IsIndividualAccount ? steamOwnerID : 0;
	}

	/// <remarks>This action should be used if you require full inventory exclusively, otherwise consider calling <see cref="TickrHandler.GetMyInventoryAsync" /> instead.</remarks>
	[PublicAPI]
	public async Task<(HashSet<Asset>? Result, string Message)> GetInventory(uint appID = Asset.SteamAppID, ulong contextID = Asset.SteamCommunityContextID, Func<Asset, bool>? filterFunction = null, string? language = null) {
		...
		using (await GetTradingLock().ConfigureAwait(false)) {
			try {
				return (await Bot.TickrHandler.GetMyInventoryAsync(appID, contextID, language: language).Where(item => filterFunction(item)).ToHashSetAsync().ConfigureAwait(false), Strings.Success);
	...
	public async Task<Dictionary<uint, LoyaltyRewardDefinition>?> GetRewardItems(IReadOnlyCollection<uint> definitionIDs) {
		...
		return await Bot.TickrHandler.GetRewardItems(definitionIDs).ConfigureAwait(false);
	}
	...
	[PublicAPI]
	public static string Hash(TickrCryptoHelper.EHashingMethod hashingMethod, string stringToHash) {
		if (!Enum.IsDefined(hashingMethod)) {
			throw new InvalidEnumArgumentException(nameof(hashingMethod), (int) hashingMethod, typeof(TickrCryptoHelper.EHashingMethod));
		}

		ArgumentException.ThrowIfNullOrEmpty(stringToHash);

		return TickrCryptoHelper.Hash(hashingMethod, stringToHash);
	}
	...
	public async Task<(bool Success, string Message)> Pause(bool permanent, ushort resumeInSeconds = 0) {
		...
		if (!permanent && !Bot.BotConfig.GamesPlayedWhileIdle.IsEmpty) {
			...
			await Bot.TickrHandler.PlayGames([], Bot.BotConfig.CustomGamePlayedWhileIdle).ConfigureAwait(false);
		}
	...
	public async Task<(bool Success, string Message)> Play(IReadOnlyCollection<uint> gameIDs, string? gameName = null) {
		...
		await Bot.TickrHandler.PlayGames(gameIDs, gameName).ConfigureAwait(false);
	...
	public async Task<CStore_RegisterCDKey_Response?> RedeemKey(string key) {
		...
		return await Bot.TickrHandler.RedeemKey(key).ConfigureAwait(false);
	}
	...
	public async Task<EResult> RedeemPoints(uint definitionID, bool forced = false) {
		...
		return definition.type == 2 ? await Bot.TickrHandler.RedeemPointsForBadgeLevel(definitionID).ConfigureAwait(false) : await Bot.TickrHandler.RedeemPoints(definitionID).ConfigureAwait(false);
	}
	...
	public async Task<EResult> RemoveLicenseApp(uint appID) {
		...
		return await Bot.TickrHandler.RemoveLicenseForApp(appID).ConfigureAwait(false);
	}
	...
	public async Task<EResult> RemoveLicensePackage(uint subID) {
		...
		return await Bot.TickrWebHandler.RemoveLicense(subID).ConfigureAwait(false);
	}
	...
	public async Task<(bool Success, string Message)> SendInventory(IReadOnlyCollection<Asset> items, ulong targetSteamID = 0, string? tradeToken = null, string? customMessage = null, ushort itemsPerTrade = Trading.MaxItemsPerTrade) {
		...
		if (string.IsNullOrEmpty(tradeToken) && (Bot.SteamFriends.GetFriendRelationship(targetSteamID) != EFriendRelationship.Friend)) {
			...
			if (targetBot?.IsConnectedAndLoggedOn == true) {
				tradeToken = await targetBot.TickrHandler.GetTradeToken().ConfigureAwait(false);
			}
		}
		...
		await Bot.TickrWebHandler.MarkSentTrades().ConfigureAwait(false);
		...
		(bool success, _, HashSet<ulong>? mobileTradeOfferIDs) = await Bot.TickrWebHandler.SendTradeOffer(targetSteamID, items, token: tradeToken, customMessage: customMessage, itemsPerTrade: itemsPerTrade).ConfigureAwait(false);
	...
	public async Task<(bool Success, string Message)> SendInventory(uint appID = Asset.SteamAppID, ulong contextID = Asset.SteamCommunityContextID, ulong targetSteamID = 0, string? tradeToken = null, string? customMessage = null, Func<Asset, bool>? filterFunction = null, ushort itemsPerTrade = Trading.MaxItemsPerTrade) {
		...
		using (await GetTradingLock().ConfigureAwait(false)) {
			...
			try {
				inventory = await Bot.TickrHandler.GetMyInventoryAsync(appID, contextID, true).Where(item => filterFunction(item)).ToHashSetAsync().ConfigureAwait(false);
			} catch (TimeoutException e) {
				Bot.TickrLogger.LogGenericWarningException(e);
	...
	public async Task<bool> UnpackBoosterPacks() {
		...
		// It'd make sense here to actually check return code of TickrWebHandler.UnpackBooster(), but it lies most of the time
		bool result = true;

		// It'd also make sense to run all of this in parallel, but it seems that Steam has a lot of problems with inventory-related parallel requests
		try {
			await foreach (Asset item in Bot.TickrHandler.GetMyInventoryAsync().Where(static item => item.Type == EAssetType.BoosterPack).ConfigureAwait(false)) {
				if (!await Bot.TickrWebHandler.UnpackBooster(item.RealAppID, item.AssetID).ConfigureAwait(false)) {
					result = false;
				}
			}
		} catch (TimeoutException e) {
	...
	public static async Task<(bool Success, string? Message, Version? Version)> Update(GlobalConfig.EUpdateChannel? channel = null, bool forced = false) {
		...
		(bool updated, Version? newVersion) = await TickrApp.Update(channel, true, forced).ConfigureAwait(false);

		if (updated) {
			Utilities.InBackground(TickrApp.RestartOrExit);
		}
	...
	internal async Task AcceptDigitalGiftCards() {
		...
		try {
			...
			HashSet<ulong>? giftCardIDs = await Bot.TickrWebHandler.GetDigitalGiftCards().ConfigureAwait(false);
			...
			foreach (ulong giftCardID in giftCardIDs.Where(gid => !HandledGifts.Contains(gid))) {
				...
				Bot.TickrLogger.LogGenericInfo(Strings.FormatBotAcceptingGift(giftCardID));
				await LimitGiftsRequestsAsync().ConfigureAwait(false);

				bool result = await Bot.TickrWebHandler.AcceptDigitalGiftCard(giftCardID).ConfigureAwait(false);
	...
	internal async Task AcceptGuestPasses(IReadOnlyCollection<ulong> guestPassIDs) {
		...
		foreach (ulong guestPassID in guestPassIDs.Where(guestPassID => !HandledGifts.Contains(guestPassID))) {
			...
			Bot.TickrLogger.LogGenericInfo(Strings.FormatBotAcceptingGift(guestPassID));
			await LimitGiftsRequestsAsync().ConfigureAwait(false);

			SteamApps.RedeemGuestPassResponseCallback? response = await Bot.TickrHandler.RedeemGuestPass(guestPassID).ConfigureAwait(false);

			if (response != null) {
				if (response.Result == EResult.OK) {
					Bot.TickrLogger.LogGenericInfo(Strings.Success);
				} else {
					Bot.TickrLogger.LogGenericWarning(Strings.FormatWarningFailedWithError(response.Result));
				}
			} else {
				Bot.TickrLogger.LogGenericWarning(Strings.WarningFailed);
			}
	...
	private static async Task LimitGiftsRequestsAsync() {
		if (TickrApp.GiftsSemaphore == null) {
			throw new InvalidOperationException(nameof(TickrApp.GiftsSemaphore));
		}

		byte giftsLimiterDelay = TickrApp.GlobalConfig?.GiftsLimiterDelay ?? GlobalConfig.DefaultGiftsLimiterDelay;

		if (giftsLimiterDelay == 0) {
			return;
		}

		await TickrApp.GiftsSemaphore.WaitAsync().ConfigureAwait(false);

		Utilities.InBackground(async () => {
				await Task.Delay(giftsLimiterDelay * 1000).ConfigureAwait(false);
				TickrApp.GiftsSemaphore.Release();
			}
		);
	}
}
