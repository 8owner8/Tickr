...
public sealed class Commands {
	...
	public static string FormatStaticResponse(string response) {
		...
		return $"<{SharedInfo.Tickr}> {response}";
	}
		...
		switch (args.Length) {
			...
			case 1:
				switch (args[0].ToUpperInvariant()) {
					...
					case "SA":
						return await ResponseStatus(access, SharedInfo.Tickr, steamID).ConfigureAwait(false);
					case "START":
			...
			default:
				switch (args[0].ToUpperInvariant()) {
					...
					case "ALA":
						return await ResponseAddLicense(access, SharedInfo.Tickr, Utilities.GetArgsAsText(args, 1, ","), steamID).ConfigureAwait(false);
					case "BALANCE":
					...
					case "OA":
						return await ResponseOwns(access, SharedInfo.Tickr, Utilities.GetArgsAsText(message, 1), steamID).ConfigureAwait(false);
	...
	internal async Task HandleMessage(ulong steamID, string message) {
		...
		string? commandPrefix = TickrApp.GlobalConfig != null ? TickrApp.GlobalConfig.CommandPrefix : GlobalConfig.DefaultCommandPrefix;
			...
			if (!message.StartsWith(commandPrefix, StringComparison.Ordinal)) {
				...
				if (!string.IsNullOrEmpty(pluginsResponse)) {
					if (!await Bot.SendMessage(steamID, pluginsResponse).ConfigureAwait(false)) {
						Bot.TickrLogger.LogGenericWarning(Strings.FormatWarningFailedWithError(nameof(Bot.SendMessage)));
						Bot.TickrLogger.LogGenericDebug(Strings.FormatContent(pluginsResponse));
					}
				}
		...
		if (feedback && !responseTask.IsCompleted) {
			if (!await Bot.SendTypingMessage(steamID).ConfigureAwait(false)) {
				Bot.TickrLogger.LogGenericWarning(Strings.FormatWarningFailedWithError(nameof(Bot.SendTypingMessage)));
			}
		...
		if (!await Bot.SendMessage(steamID, response).ConfigureAwait(false)) {
			Bot.TickrLogger.LogGenericWarning(Strings.FormatWarningFailedWithError(nameof(Bot.SendMessage)));
			Bot.TickrLogger.LogGenericDebug(Strings.FormatContent(response));
		}
	}
	...
	private async Task<IReadOnlyDictionary<uint, string>?> FetchGamesOwned(bool cachedOnly = false) {
		...
		Dictionary<uint, string>? gamesOwned = await Bot.TickrHandler.GetOwnedGames(Bot.SteamID).ConfigureAwait(false);
	...
	private async Task<string?> Response2FAConfirm(EAccess access, bool confirm) {
		...
		if (!Bot.HasMobileAuthenticator) {
			return FormatBotResponse(Strings.BotNoTickrAuthenticator);
		}
		...
		foreach (string entry in entries) {
			...
			switch (type) {
				case EGameIdentifier.Application: {
					HashSet<uint>? packageIDs = TickrApp.GlobalDatabase?.GetPackageIDs(gameID, Bot.OwnedPackages.Keys, 1);
	...
	private static string? ResponseEncrypt(EAccess access, string cryptoMethodText, string stringToEncrypt) {
		...
		if (!Enum.TryParse(cryptoMethodText, true, out TickrCryptoHelper.ECryptoMethod cryptoMethod)) {
	...
	private static string? ResponseHash(EAccess access, string hashingMethodText, string stringToHash) {
		...
		if (!Enum.TryParse(hashingMethodText, true, out TickrCryptoHelper.EHashingMethod hashingMethod)) {
	...
	private string? ResponseInput(EAccess access, string propertyName, string inputValue) {
		...
		bool headless = Program.Service || (TickrApp.GlobalConfig?.Headless ?? GlobalConfig.DefaultHeadless);

		if (!headless) {
			return FormatBotResponse(Strings.ErrorFunctionOnlyInHeadlessMode);
		}

		if (!Enum.TryParse(propertyName, true, out TickrApp.EUserInputType inputType) || (inputType == TickrApp.EUserInputType.None) || !Enum.IsDefined(inputType)) {
	...
	private async Task<string?> ResponseInventory(EAccess access) {
		...
		ImmutableDictionary<uint, InventoryAppData>? inventory = await Bot.TickrWebHandler.GetInventoryContextData().ConfigureAwait(false);
	...
	private async Task<string?> ResponseLevel(EAccess access) {
		...
		uint? level = await Bot.TickrHandler.GetLevel().ConfigureAwait(false);
		...
		foreach (string entry in entries) {
			...
			switch (type) {
				case EGameIdentifier.Application when uint.TryParse(game, out uint appID) && (appID > 0):
					HashSet<uint>? packageIDs = TickrApp.GlobalDatabase?.GetPackageIDs(appID, Bot.OwnedPackages.Keys, 1);
	...
	private async Task<string?> ResponsePlay(EAccess access, HashSet<uint> gameIDs, string? gameName = null) {
		...
		if (gameIDs.Count > TickrHandler.MaxGamesPlayedConcurrently) {
	...
	private async Task<string?> ResponsePlay(EAccess access, string targetGameIDs) {
		...
		HashSet<uint> gamesToPlay = [with(Math.Min(games.Length, TickrHandler.MaxGamesPlayedConcurrently))];
		...
		foreach (string game in games) {
			...
			if (gamesToPlay.Count >= TickrHandler.MaxGamesPlayedConcurrently) {
				return FormatBotResponse(Strings.FormatWarningFailedWithError($"{nameof(gamesToPlay)} > {TickrHandler.MaxGamesPlayedConcurrently}"));
			}
	...
	private async Task<string?> ResponsePointsBalance(EAccess access) {
		...
		long? points = await Bot.TickrHandler.GetPointsBalance().ConfigureAwait(false);
	...
	private async Task<string?> ResponsePrivacy(EAccess access, string privacySettingsText) {
				...
				case 6:
					...
					switch (privacySetting) {
						...
						case ECommunityPrivacy.Public:
							...
							break;
						default:
							Bot.TickrLogger.LogGenericError(Strings.FormatWarningUnknownValuePleaseReport(nameof(privacySetting), privacySetting));
					...
					break;
				default:
					Bot.TickrLogger.LogGenericError(Strings.FormatWarningUnknownValuePleaseReport(nameof(index), index));
		...
		return FormatBotResponse(await Bot.TickrWebHandler.ChangePrivacySettings(userPrivacy).ConfigureAwait(false) ? Strings.Success : Strings.WarningFailed);
	}
						...
						if ((currentBot == Bot) && redeemFlags.HasFlag(ERedeemFlags.SkipInitial)) {
							...
							if ((result == EResult.Timeout) || (purchaseResultDetail == EPurchaseResultDetail.Timeout)) {
								...
								if ((purchaseResultDetail == EPurchaseResultDetail.CannotRedeemCodeFromClient) || ((purchaseResultDetail == EPurchaseResultDetail.BadActivationCode) && assumeWalletKeyOnBadActivationCode)) {
									...
									(EResult Result, EPurchaseResultDetail? PurchaseResult, string? BalanceText)? walletResult = await currentBot.TickrWebHandler.RedeemWalletKey(key).ConfigureAwait(false);
	...
	private static async Task<string?> ResponseUpdate(EAccess access, string? channelText = null) {
		...
		bool forced = false;
		GlobalConfig.EUpdateChannel channel = TickrApp.GlobalConfig?.UpdateChannel ?? GlobalConfig.DefaultUpdateChannel;
	...
	private string? ResponseVersion(EAccess access) {
		...
		return access >= EAccess.FamilySharing ? FormatBotResponse(Strings.FormatBotVersion(SharedInfo.Tickr, SharedInfo.Version)) : null;
	}
