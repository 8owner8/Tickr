...
public sealed class MobileAuthenticator : IDisposable {
	...
	private readonly TickrCacheable<string> CachedDeviceID;
	...
	[JsonConstructor]
	private MobileAuthenticator() => CachedDeviceID = new TickrCacheable<string>(ResolveDeviceID);
	...
	internal string? GenerateTokenForTime(ulong time) {
		...
		if (!Convert.TryFromBase64String(SharedSecret, sharedSecret, out int bytesWritten)) {
			Bot.TickrLogger.LogGenericError(Strings.FormatErrorIsInvalid(nameof(SharedSecret)));
...
#pragma warning restore CA5350 // This is actually a fair warning, but there is nothing we can do about Steam using weak cryptographic algorithms
	...
	internal async Task<ImmutableHashSet<Confirmation>?> GetConfirmations() {
		...
		if (string.IsNullOrEmpty(deviceID)) {
			Bot.TickrLogger.LogGenericError(Strings.FormatWarningFailedWithError(nameof(deviceID)));
		...
		if (string.IsNullOrEmpty(confirmationHash)) {
			Bot.TickrLogger.LogNullError(confirmationHash);

			return null;
		}

		ConfirmationsResponse? response = await Bot.TickrWebHandler.GetConfirmations(deviceID, confirmationHash, time).ConfigureAwait(false);
		...
		foreach (Confirmation confirmation in response.Confirmations.Where(static confirmation => (confirmation.ConfirmationType == EMobileConfirmationType.Invalid) || !Enum.IsDefined(confirmation.ConfirmationType))) {
			Bot.TickrLogger.LogGenericError(Strings.FormatWarningUnknownValuePleaseReport(nameof(confirmation.ConfirmationType), $"{confirmation.ConfirmationType} ({confirmation.ConfirmationTypeName ?? "null"})"));
		}
	...
	internal async Task<ulong> GetSteamTime() {
		...
		try {
			...
			ulong serverTime = await Bot.TickrHandler.GetServerTime().ConfigureAwait(false);
	...
	internal async Task<bool> HandleConfirmations(IReadOnlyCollection<Confirmation> confirmations, bool accept) {
		...
		bool? result = await Bot.TickrWebHandler.HandleConfirmations(deviceID, confirmationHash, time, confirmations, accept).ConfigureAwait(false);
		...
		foreach (Confirmation confirmation in confirmations) {
			bool? confirmationResult = await Bot.TickrWebHandler.HandleConfirmation(deviceID, confirmationHash, time, confirmation.ID, confirmation.Nonce, accept).ConfigureAwait(false);
	...
	private string? GenerateConfirmationHash(ulong time, string? tag = null) {
		...
		try {
			...
			Bot.TickrLogger.LogGenericError(Strings.FormatErrorIsInvalid(nameof(IdentitySecret)));
...
#pragma warning restore CA5350 // This is actually a fair warning, but there is nothing we can do about Steam using weak cryptographic algorithms
	...
	private static async Task LimitConfirmationsRequestsAsync() {
		if (TickrApp.ConfirmationsSemaphore == null) {
			throw new InvalidOperationException(nameof(TickrApp.ConfirmationsSemaphore));
		}

		byte confirmationsLimiterDelay = TickrApp.GlobalConfig?.ConfirmationsLimiterDelay ?? GlobalConfig.DefaultConfirmationsLimiterDelay;

		if (confirmationsLimiterDelay == 0) {
			return;
		}

		await TickrApp.ConfirmationsSemaphore.WaitAsync().ConfigureAwait(false);

		Utilities.InBackground(async () => {
				await Task.Delay(confirmationsLimiterDelay * 1000).ConfigureAwait(false);
				TickrApp.ConfirmationsSemaphore.Release();
			}
		);
	}
	...
	private async Task<(bool Success, string? Result)> ResolveDeviceID(CancellationToken cancellationToken = default) {
		...
		string? deviceID = await Bot.TickrHandler.GetTwoFactorDeviceIdentifier(Bot.SteamID).ConfigureAwait(false);
