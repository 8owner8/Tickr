...
public sealed class BotConfig {
	...
	[PublicAPI]
	public const TickrCryptoHelper.ECryptoMethod DefaultPasswordFormat = TickrCryptoHelper.ECryptoMethod.PlainText;
	...
	[JsonDisallowNull]
	[JsonInclude]
	[MaxLength(TickrHandler.MaxGamesPlayedConcurrently)]
	...
	[JsonInclude]
	public TickrCryptoHelper.ECryptoMethod PasswordFormat { get; internal set; } = DefaultPasswordFormat;
	...
	internal (bool Valid, string? ErrorMessage) CheckValidation() {
		...
		if (GamesPlayedWhileIdle.Count > TickrHandler.MaxGamesPlayedConcurrently) {
			return (false, Strings.FormatErrorConfigPropertyInvalid(nameof(GamesPlayedWhileIdle), $"{nameof(GamesPlayedWhileIdle.Count)} {GamesPlayedWhileIdle.Count} > {TickrHandler.MaxGamesPlayedConcurrently}"));
		}
	...
	internal async Task<string?> GetDecryptedSteamPassword() {
		...
		if (PasswordFormat == TickrCryptoHelper.ECryptoMethod.PlainText) {
		...
		string? result = await TickrCryptoHelper.Decrypt(PasswordFormat, SteamPassword).ConfigureAwait(false);

		if (string.IsNullOrEmpty(result)) {
			TickrApp.TickrLogger.LogGenericError(Strings.FormatErrorIsInvalid(nameof(SteamPassword)));
	...
	internal static async Task<(BotConfig? BotConfig, string? LatestJson)> Load(string filePath) {
		...
		if (botConfig == null) {
			TickrApp.TickrLogger.LogNullError(botConfig);
		...
		if (!valid) {
			if (!string.IsNullOrEmpty(errorMessage)) {
				TickrApp.TickrLogger.LogGenericError(errorMessage);
			}
		...
		switch (botConfig.PasswordFormat) {
			case TickrCryptoHelper.ECryptoMethod.AES when TickrCryptoHelper.HasDefaultCryptKey:
				TickrApp.TickrLogger.LogGenericError(Strings.FormatWarningDefaultCryptKeyUsedForEncryption(botConfig.PasswordFormat, nameof(SteamPassword)));

				break;
			case TickrCryptoHelper.ECryptoMethod.ProtectedDataForCurrentUser when TickrCryptoHelper.HasDefaultCryptKey:
				TickrApp.TickrLogger.LogGenericInfo(Strings.FormatWarningDefaultCryptKeyUsedForHashing(botConfig.PasswordFormat, nameof(SteamPassword)));
