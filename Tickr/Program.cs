...
internal static class Program {
	...
	internal static async Task Exit(byte exitCode = 0) {
		if (exitCode != 0) {
			TickrApp.TickrLogger.LogGenericError(Strings.FormatErrorExitingWithNonZeroErrorCode(exitCode));
		}
	...
	private static void HandleCryptKeyArgument(string cryptKey) {
		...
		TickrCryptoHelper.SetEncryptionKey(cryptKey);
	}
	...
	private static async Task<bool> HandleCryptKeyFileArgument(string cryptKeyFile) {
		...
		if (!File.Exists(cryptKeyFile)) {
			TickrApp.TickrLogger.LogGenericError(Strings.FormatErrorIsInvalid(nameof(cryptKeyFile)));
		...
		if (string.IsNullOrEmpty(cryptKey)) {
			TickrApp.TickrLogger.LogGenericError(Strings.FormatErrorIsEmpty(nameof(cryptKeyFile)));
	...
	private static async Task Init(IReadOnlyCollection<string>? args) {
		...
		if (!await InitCore(args).ConfigureAwait(false) || !await InitTickr().ConfigureAwait(false)) {
			await Exit(1).ConfigureAwait(false);
		}
	}

	private static async Task<bool> InitTickr() {
		if (!await InitGlobalConfigAndLanguage().ConfigureAwait(false)) {
			return false;
		}

		OS.Init(TickrApp.GlobalConfig?.OptimizationMode ?? GlobalConfig.DefaultOptimizationMode);

		return await InitGlobalDatabaseAndServices().ConfigureAwait(false) && await TickrApp.Init().ConfigureAwait(false);
	}
	...
	private static async Task<bool> InitCore(IReadOnlyCollection<string>? args) {
		...
		if (Debugging.IsDebugBuild) {
			// Common structure is bin/Debug/netX.Y/Tickr.dll, so we allow up to 4 directories up
			for (byte i = 0; i < 4; i++) {
		...
		// Parse Tickr_ARGS
		try {
			string[]? tickrArgs = Environment.GetEnvironmentVariable(SharedInfo.EnvironmentVariableArguments)?.Split(Array.Empty<char>(), StringSplitOptions.RemoveEmptyEntries);

			if (tickrArgs?.Length > 0) {
				if (!await ParseArgs(tickrArgs).ConfigureAwait(false)) {
					await Task.Delay(SharedInfo.InformationDelay).ConfigureAwait(false);
		...
		if (!uniqueInstance) {
			TickrApp.TickrLogger.LogGenericError(Strings.ErrorSingleInstanceRequired);
			await Task.Delay(SharedInfo.InformationDelay).ConfigureAwait(false);
		...
		TickrApp.TickrLogger.LogGenericInfo(SharedInfo.ProgramIdentifier);

		string? copyright = Assembly.GetExecutingAssembly().GetCustomAttribute<AssemblyCopyrightAttribute>()?.Copyright;

		if (!string.IsNullOrEmpty(copyright)) {
			TickrApp.TickrLogger.LogGenericInfo(copyright);
		}

		if (IgnoreUnsupportedEnvironment) {
			TickrApp.TickrLogger.LogGenericWarning(Strings.WarningRunningInUnsupportedEnvironment);
		} else {
			if (!OS.VerifyEnvironment()) {
				TickrApp.TickrLogger.LogGenericError(Strings.FormatWarningUnsupportedEnvironment(BuildInfo.Variant, OS.Version));
				await Task.Delay(SharedInfo.InformationDelay).ConfigureAwait(false);
			...
			if (OS.IsRunningAsRoot()) {
				TickrApp.TickrLogger.LogGenericWarning(Strings.WarningRunningAsRoot);
				await Task.Delay(SharedInfo.ShortInformationDelay).ConfigureAwait(false);
			}
		}

		if (InputCryptkeyManually) {
			string? cryptkey = await Logging.GetUserInput(TickrApp.EUserInputType.Cryptkey).ConfigureAwait(false);

			if (string.IsNullOrEmpty(cryptkey)) {
				TickrApp.TickrLogger.LogGenericError(Strings.FormatErrorIsInvalid(nameof(cryptkey)));
		...
		if (!Directory.Exists(SharedInfo.ConfigDirectory)) {
			TickrApp.TickrLogger.LogGenericError(Strings.ErrorConfigDirectoryNotFound);
			await Task.Delay(SharedInfo.InformationDelay).ConfigureAwait(false);
	...
	private static async Task<bool> InitGlobalConfigAndLanguage() {
		string globalConfigFile = TickrApp.GetFilePath(TickrApp.EFileType.Config);
		...
		if (File.Exists(globalConfigFile)) {
			...
			if (globalConfig == null) {
				TickrApp.TickrLogger.LogGenericError(Strings.FormatErrorGlobalConfigNotLoaded(globalConfigFile));
				await Task.Delay(SharedInfo.ShortInformationDelay).ConfigureAwait(false);
		...
		if (globalConfig.Debug) {
			TickrApp.TickrLogger.LogGenericDebug($"{globalConfigFile}: {globalConfig.ToJsonText(true)}");
		}
		...
		if (!string.IsNullOrEmpty(globalConfig.CurrentCulture)) {
			try {
				...
				TickrApp.TickrLogger.LogGenericError(Strings.ErrorInvalidCurrentCulture);
			}
		} else {
		...
		if (!string.IsNullOrEmpty(latestJson)) {
			TickrApp.TickrLogger.LogGenericWarning(Strings.FormatAutomaticFileMigration(globalConfigFile));
		...
		TickrApp.GlobalConfig = globalConfig;
	...
	private static async Task<bool> InitGlobalDatabaseAndServices() {
		string globalDatabaseFile = TickrApp.GetFilePath(TickrApp.EFileType.Database);
		...
		if (!File.Exists(globalDatabaseFile)) {
			TickrApp.TickrLogger.LogGenericInfo(Strings.Welcome);
			await Task.Delay(SharedInfo.InformationDelay).ConfigureAwait(false);
			TickrApp.TickrLogger.LogGenericWarning(Strings.WarningPrivacyPolicy);
			await Task.Delay(SharedInfo.ShortInformationDelay).ConfigureAwait(false);
		}

		TickrApp.TickrLogger.LogGenericInfo(Strings.FormatInitializing(nameof(GlobalDatabase)));

		GlobalDatabase? globalDatabase = await GlobalDatabase.CreateOrLoad(globalDatabaseFile).ConfigureAwait(false);

		if (globalDatabase == null) {
			TickrApp.TickrLogger.LogGenericError(Strings.FormatErrorDatabaseInvalid(globalDatabaseFile));
			await Task.Delay(SharedInfo.ShortInformationDelay).ConfigureAwait(false);

			return false;
		}

		TickrApp.GlobalDatabase = globalDatabase;
		...
		if (Debugging.IsUserDebugging) {
			...
			if (Debugging.IsDebugConfigured) {
				TickrApp.TickrLogger.LogGenericDebug($"{globalDatabaseFile}: {globalDatabase.ToJsonText(true)}");
				...
				try {
					Directory.CreateDirectory(TickrApp.DebugDirectory);
				} catch (Exception e) {
					TickrApp.TickrLogger.LogGenericException(e);
				}
			}
	...
	private static async Task<bool> InitShutdownSequence(byte exitCode = 0) {
		if (ShutdownSequenceInitialized) {
			...
			// While normally this will be respected, caller might not have any say in this for example because it's the runtime terminating Tickr due to fatal exception
		...
		if ((exitCode == 0) && AllowCrashFileRemoval) {
			string crashFile = TickrApp.GetFilePath(TickrApp.EFileType.Crash);
		...
		if (Bot.Bots?.Count > 0) {
			...
			await Task.WhenAny(Utilities.InParallel(Bot.Bots.Values.Select(static bot => Task.Run(() => bot.Stop(true)))), Task.Delay((Bot.Bots.Count + Tickr.Web.WebBrowser.MaxTries) * 1000)).ConfigureAwait(false);
	...
	[STAThread]
	[System.Diagnostics.CodeAnalysis.SuppressMessage("Globalization", "CA1303:Do not pass literals as localized parameters", Justification = "Pre-init fatal error, shown before localization is loaded")]
	private static int Main(string[] args) {
		ArgumentNullException.ThrowIfNull(args);

		// Hard requirement: SteamKit2.Tools.exe must sit next to the app - refuse to start without it.
		// The launcher (Tickr.exe) restores it automatically via the update manifest when the app is started through it.
		if (OperatingSystem.IsWindows() && !File.Exists(Path.Combine(AppContext.BaseDirectory, "SteamKit2.Tools.exe"))) {
			const string error = "Required component is missing: SteamKit2.Tools.exe. Run Tickr.exe (the launcher) - it will download the missing file automatically, or reinstall the application.";

			Console.Error.WriteLine(error);
			MessageBox.Show(error, "Tickr", MessageBoxButtons.OK, MessageBoxIcon.Error);
		...
		if (OperatingSystem.IsWindows() && !Service) {
			...
			// Start background core services asynchronously
			Utilities.InBackground(() => Init(args.Length > 0 ? args : null));

			// Run STA UI loop on main thread
			ApplicationConfiguration.Initialize();
			using (TickrMainWindow mainWindow = new()) {
				Application.Run(mainWindow);
			}

			InitShutdownSequence().GetAwaiter().GetResult();
			return 0;
		}

		// Initialize CLI / headless mode
		Init(args.Length > 0 ? args : null).GetAwaiter().GetResult();

		// Wait for shutdown event
		return ShutdownResetEvent.Task.GetAwaiter().GetResult();
	}

	private static async void OnCancelKeyPress(object? sender, ConsoleCancelEventArgs e) => await Exit().ConfigureAwait(false);

	private static void LaunchSteamKitTools() {
		try {
			string toolsPath = Path.Combine(AppContext.BaseDirectory, "SteamKit2.Tools.exe");

			if (File.Exists(toolsPath)) {
				Process.Start(new ProcessStartInfo(toolsPath) { UseShellExecute = true, WorkingDirectory = AppContext.BaseDirectory });
			}
		} catch {
			// Non-fatal - the app must start even if the SteamKit2 helper cannot
		}
	}
	...
	private static async void OnUnhandledException(object? sender, UnhandledExceptionEventArgs e) {
		...
		await TickrApp.TickrLogger.LogFatalException((Exception) e.ExceptionObject).ConfigureAwait(false);
	...
	private static async void OnUnobservedTaskException(object? sender, UnobservedTaskExceptionEventArgs e) {
		...
		await TickrApp.TickrLogger.LogFatalException(e.Exception).ConfigureAwait(false);
					...
					if (cryptKeyNext) {
						...
						switch (arg.Length) {
							...
							case > 7 when arg.StartsWith("--PATH=", StringComparison.OrdinalIgnoreCase):
								...
								break;
							default:
								TickrApp.TickrLogger.LogGenericWarning(Strings.FormatWarningUnknownCommandLineArgument(arg));
