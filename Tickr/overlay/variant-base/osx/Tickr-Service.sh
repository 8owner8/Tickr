...
while :; do
	if [ "$SERVICE" -eq 1 ] || ([ -f "$CONFIG_PATH" ] && grep -Eq '"Headless":\s+?true' "$CONFIG_PATH"); then
		...
		"$BINARY" "$@" & # Start Tickr in the background, trap will work properly due to non-blocking call
		...
		"$BINARY" "$@" # Start Tickr in the foreground, trap sadly won't work until process exit
	fi
