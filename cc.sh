...
PRINT_USAGE() {
	echo "Usage: $0 [--clean] [--no-analysis] [--no-pull] [--no-shared-compilation] [--no-test] [debug/release]"
}
...
dotnet publish "$MAIN_PROJECT" -o "$OUT_TICKR" $DOTNET_FLAGS $PUBLISH_FLAGS
...
if [ -n "${STEAM_TOKEN_DUMPER_TOKEN-}" ] && [ -f "${STEAM_TOKEN_DUMPER_NAME}/SharedInfo.cs" ] && command -v git >/dev/null; then
	...
	dotnet publish "$STEAM_TOKEN_DUMPER_NAME" -o "${OUT_TICKR}/plugins/${STEAM_TOKEN_DUMPER_NAME}" $DOTNET_FLAGS $PUBLISH_FLAGS
	git checkout -- "${STEAM_TOKEN_DUMPER_NAME}/SharedInfo.cs"
else
...
for plugin in $PLUGINS; do
	...
	dotnet publish "$plugin" -o "${OUT_TICKR}/plugins/${plugin}" $DOTNET_FLAGS $PUBLISH_FLAGS
done
