...
if [ "$SERVICE" -eq 1 ] || ([ -f "$CONFIG_PATH" ] && grep -Eq '"Headless":\s+?true' "$CONFIG_PATH"); then
	...
	dotnet "$BINARY" $BINARY_ARGS & # Start Tickr in the background, trap will work properly due to non-blocking call
	...
	dotnet "$BINARY" $BINARY_ARGS # Start Tickr in the foreground, trap won't work until process exit
fi
