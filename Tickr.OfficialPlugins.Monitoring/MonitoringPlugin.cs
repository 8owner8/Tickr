...
internal sealed class MonitoringPlugin : OfficialPlugin, IBot, IBotTradeOfferResults, IDisposable, IOfficialGitHubPluginUpdates, IWebInterface, IWebServiceProvider {
	...
	private const string MetricNamePrefix = "tickr";
	...
	private static bool Enabled => TickrApp.GlobalConfig?.IPC ?? GlobalConfig.DefaultIPC;
	...
	private void InitializeMeter() {
		...
		Meter.CreateObservableGauge(
			...
			static () => BuildInfo,
			description: "Build information about Tickr in form of label values"
		);

		Meter.CreateObservableGauge(
			$"{MetricNamePrefix}.runtime.info",
			static () => RuntimeInfo,
			description: "Runtime information about Tickr in form of label values"
		);

		Meter.CreateObservableGauge(
			$"{MetricNamePrefix}.ipc.banned.ips",
			static () => ApiAuthenticationMiddleware.GetCurrentlyBannedIPs().Count(),
			description: "Number of IP addresses currently banned by Tickr's IPC module"
		);

		Meter.CreateObservableGauge(
			$"{MetricNamePrefix}.active.plugins",
			static () => PluginMeasurements,
			description: "Number of plugins currently loaded in Tickr"
		);
		...
		Meter.CreateObservableGauge(
			...
			description: "Number of bots that are currently loaded in Tickr"
		);
		...
		Meter.CreateObservableCounter(
			...
			description: "Trade offers per bot and action taken by Tickr"
		);
